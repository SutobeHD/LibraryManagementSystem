...
from .downloader.orchestrator import enqueue_fetch
from .downloader.orchestrator import get_job as get_unified_job
from .downloader.resolver import resolve as unified_resolve
from .downloader.search import search as unified_search
...
def validate_audio_path(path_str: str) -> Path:
    ...
    is_allowed = any(file_path.is_relative_to(root) for root in ALLOWED_AUDIO_ROOTS)
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "errors": [
                {"field": list(e["loc"]), "message": e["msg"], "type": e["type"]} for e in errors
            ],
        },
    )
...
class SetReq(BaseModel):
    ...
    def _enforce_caps(self) -> "SetReq":
        ...
        try:
            serialized = _json_caps.dumps(self.model_dump(), separators=(",", ":"), default=str)
...
def get_label_tracks(aid: str):
    return db.get_tracks_by_label(aid)


@app.get("/api/album/{aid}/tracks")
def get_album_tracks(aid: str):
    return db.get_tracks_by_album(aid)
...
def get_cues(tid: str):
    return db.get_track_cues(tid)


@app.get("/api/track/{tid}/beatgrid")
def get_beatgrid(tid: str):
    return db.get_track_beatgrid(tid)
...
async def analyze_track(tid: str):
    ...
    try:
        ...
        result = await loop.run_in_executor(
            AudioAnalyzer.get_executor(), BeatAnalyzer.analyze, path
        )
...
def save_cues(r: CueReq):
    return {"status": "success" if db.save_track_cues(r.track_id, r.cues) else "error"}


@app.post("/api/track/grid/save", dependencies=[Depends(require_session)])
def save_grid(r: GridReq):
    return {"status": "success" if db.save_track_beatgrid(r.track_id, r.beat_grid) else "error"}
...
def batch_up(r: BatchReq):
    return {"status": "success" if db.update_tracks_metadata(r.track_ids, r.updates) else "error"}
...
def rename_pl(r: RenamePlReq):
    return {"status": "success" if db.rename_playlist(r.pid, r.name) else "error"}


@app.post("/api/playlists/move", dependencies=[Depends(require_session)])
def move_pl(r: MovePlReq):
    return {
        "status": "success"
        if db.move_playlist(r.pid, r.parent_id, r.target_id, r.position)
        else "error"
    }


@app.post("/api/playlists/delete", dependencies=[Depends(require_session)])
def delete_pl(r: DeletePlReq):
    return {"status": "success" if db.delete_playlist(r.pid) else "error"}


@app.post("/api/playlists/add-track", dependencies=[Depends(require_session)])
def add_trk_pl(r: TrackPlReq):
    return {"status": "success" if db.add_track_to_playlist(r.pid, r.track_id) else "error"}
...
def create_folder_pl(r: CreatePlReq):
    ...
    try:
        node = (
            db.create_folder(r.name, r.parent_id)
            if hasattr(db, "create_folder")
            else db.create_playlist(r.name, r.parent_id, is_folder=True)
        )
        if node:
            return {
                "status": "success",
                "id": str(node.get("ID") if isinstance(node, dict) else node),
            }
...
def get_dupes():
    return LibraryTools.find_duplicates()
...
async def batch_comment(r: BatchCommentReq):
    try:
        ...
        result = subprocess.check_output(cmd, text=True, stderr=subprocess.STDOUT, timeout=600)
...
def get_lib_status():
    return {
        ...
        "loading_current_item": getattr(db.active_db, "loading_status", "Idle")
        if db.active_db
        else "Idle",
    }
    ...
    def _scan():
        ...
        for audio_file in scan_path.rglob("*"):
            if (
                ...
                or audio_file.suffix.lower() not in folder_watcher.AUDIO_EXTENSIONS
            ):
                continue
            try:
                ...
                skipped += 1
        logger.info(
            "[Scan] Folder scan complete: path=%s imported=%d skipped=%d",
            path_str,
            imported,
            skipped,
        )
        ...
        try:
            ...
            for tid_local in collected_track_ids:
                try:
                    ...
                    logger.debug(
                        "[Drop] add_track_to_playlist(%s, %s) failed: %s", pid, tid_local, exc
                    )
            logger.info(
                "[Drop] Bundled %d tracks into playlist '%s' (pid=%s)",
                counters["linked"],
                playlist_name,
                pid,
            )
...
async def get_artwork(path: str):
    ...
    if not is_allowed:
        ...
        raise HTTPException(
            status_code=403, detail="Access denied: path outside allowed artwork directories"
        )
...
async def render(r: ExportRequest):
    try:
        ...
        tid = await asyncio.to_thread(
            AudioEngine.render_segment, r.source_path, r.cuts, safe_name, r.fade_in, r.fade_out
        )
        return {
            "status": "success",
            "track_id": tid,
...
def import_audio(files: list[UploadFile] = File(...), wait: bool = False):
    ...
    for file in files:
        ...
        try:
            ...
            if (
                not file.filename
                or Path(file.filename).suffix.lower() not in ALLOWED_AUDIO_EXTENSIONS
            ):
                results.append(
                    {
...
def usb_format_preview(r: UsbFormatPreviewReq):
    ...
    drive_path = Path(
        drive if drive.endswith(("\\", "/")) else drive + ("\\" if ":" in drive else "")
    )
...
def usb_format_confirm(r: UsbFormatConfirmReq):
    ...
    with _format_tokens_lock:
        ...
        if not meta:
            raise HTTPException(
                403, "Invalid or expired confirmation token. Re-open the format dialog."
            )
        if meta.get("used"):
...
async def analyze_track_full(tid: str, req: AnalyzeFullReq = AnalyzeFullReq()):
    ...
    if not hasattr(db, "get_analysis_writer"):
        raise HTTPException(
            400, "Analysis-to-DB requires live database mode. Switch to live mode first."
        )
...
def get_analyze_capabilities():
    ...
    if hasattr(db, "tracks"):
        ...
        unanalyzed_count = len([t for t in db.tracks.values() if not t.get("BPM") or t["BPM"] <= 0])
...
async def soundcloud_download(data: ScDownloadRequest, request: Request):
    ...
    if not os.access(str(sc_dir), os.W_OK):
        raise HTTPException(
            status_code=403, detail=f"No write permission for download directory: {sc_dir}."
        )
    ...
    if not sc_track_id:
        if not permalink_url:
            raise HTTPException(
                status_code=400, detail="Either 'url' or 'sc_track_id' is required."
            )
        logger.info("[SC-DL API] Resolving URL: %s", permalink_url)
        try:
            track_meta = await asyncio.to_thread(
                SoundCloudPlaylistAPI.resolve_track_from_url,
                permalink_url,
                auth_token,
            )
        ...
        if not track_meta:
            raise HTTPException(
                status_code=400, detail="URL does not point to a valid SoundCloud track."
            )
...
async def soundcloud_download_playlist(r: ScDownloadPlaylistReq):
    ...
    try:
        if r.is_likes:
            ...
            sc_tracks = await asyncio.to_thread(
                SoundCloudPlaylistAPI.get_full_playlist_tracks,
                r.playlist_id,
                auth_token,
            )
    except AuthExpiredError:
    ...
    if not sc_tracks:
        return {
            ...
            "skipped": 0,
            "task_ids": [],
            "message": "Playlist is empty or all tracks unavailable.",
        }
...
# ─── Unified Multi-Source Downloader API ──────────────────────────────────────
@app.post(
    "/api/downloads/unified/resolve",
    response_model=ResolveResponse,
    dependencies=[Depends(require_session)],
)
async def unified_downloads_resolve(req: ResolveRequest) -> ResolveResponse:
    """Resolve one identifier into ranked, match-gated candidates."""
    if not req.identifier.strip():
        raise HTTPException(status_code=400, detail="identifier must not be empty")
    try:
        return await unified_resolve(req)
    except (httpx.HTTPError, httpx.TimeoutException) as exc:
        logger.warning("[unified-dl] resolve transport failure: %s", exc)
        raise HTTPException(status_code=502, detail="Upstream provider transport failure") from exc
...
async def unified_downloads_search(req: SearchRequest) -> SearchResponse:
    """Free-form query into ISRC-deduped, quality-ranked hit clusters."""
    if not req.query.strip():
        raise HTTPException(status_code=400, detail="query must not be empty")
    try:
        return await unified_search(req)
    except (httpx.HTTPError, httpx.TimeoutException) as exc:
        logger.warning("[unified-dl] search transport failure: %s", exc)
        raise HTTPException(status_code=502, detail="Upstream provider transport failure") from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def unified_downloads_fetch(req: FetchRequest) -> FetchResponse:
    """Commit a chosen candidate to download — enqueues a background job."""
    if req.candidate_index < 0:
        raise HTTPException(status_code=400, detail="candidate_index must be non-negative")
    if not req.request_id.strip():
        raise HTTPException(status_code=400, detail="request_id must not be empty")
    try:
        return enqueue_fetch(req)
    except KeyError as exc:
        raise HTTPException(
            status_code=400, detail="Unknown request_id or candidate_index out of range"
        ) from exc
    except RuntimeError as exc:
        raise HTTPException(
            status_code=503, detail="Downloader unavailable — try again shortly"
        ) from exc


@app.get(
    "/api/downloads/unified/jobs/{job_id}",
    response_model=JobStatus,
    dependencies=[Depends(require_session)],
)
async def unified_downloads_job_status(job_id: str) -> JobStatus:
    """Poll the state of an in-flight (or finished) unified download job."""
    job = get_unified_job(job_id)
    if job is None:
...
async def get_download_history(
    ...
    limit = min(limit, 500)
    effective_device_id = (
        download_registry.get_current_device_id() if this_device_only else device_id
    )
    try:
...
async def merge_soundcloud_playlists(r: ScMergeReq, request: Request):
    ...
    try:
        ...
        logger.info(
            f"[SC] Merging into '{merged_name}' (pid={merged_pid}), delete_originals={r.delete_originals}"
        )
        ...
        for pl in selected_pls:
            ...
            for sc_track in sc_tracks:
                matched = engine._fuzzy_match_track(
                    sc_track.get("title", ""), sc_track.get("artist", ""), local_tracks
                )
                if matched:
...
async def phrase_generate(body: PhraseGenerateRequest):
    ...
    try:
        ...
        if not beats:
            ...
            return {
                "status": "ok",
                "data": {"cues": [], "warning": "No beat grid found — analyse track first"},
            }
...
async def phrase_commit(body: PhraseCommitRequest):
    ...
    """
    logger.info("[PHRASE] commit: track_id=%d cues=%d", body.track_id, len(body.cues))
    try:
        if not db.loaded:
            raise HTTPException(status_code=400, detail="Library not loaded")
...
async def _run_duplicate_scan(job_id: str, track_paths: list[str]) -> None:
    ...
    try:
        ...
        logger.info(
            "[DUP] job %s: fingerprinted %d/%d tracks", job_id, len(fingerprints), len(track_paths)
        )
                    ...
                    try:
                        for t in db.get_tracks() or []:
                            ...
                            if loc and (loc == p or Path(loc) == Path(p)):
                                ...
                                track_meta["bitrate"] = int(
                                    t.get("BitRate") or t.get("bitrate") or 0
                                )
                                track_meta["play_count"] = int(
                                    t.get("PlayCount") or t.get("play_count") or 0
                                )
                                break
                    except Exception as meta_exc:
...
async def duplicates_merge(body: DuplicateMergeRequest):
    ...
    logger.info(
        ...
        body.keep_path,
        len(body.remove_paths),
        body.merge_play_counts,
    )
    try:
