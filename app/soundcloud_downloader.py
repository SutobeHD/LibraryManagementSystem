...
def _resolve_official_download_url(
    ...
    try:
        ...
        with requests.get(
            ...
            final_url = resp.url
            # SSRF guard: only accept resolved URLs that land on a known SC
            # CDN host. Without this, a hostile redirect could point us at
            # localhost / cloud metadata / an attacker host — and the
            # subsequent _stream_file_to_temp call would attach the OAuth
            # token to that request and write the response to disk.
            if not _is_allowed_cdn_url(final_url):
                logger.warning(
                    "[SC-DL] Rejected resolved URL outside CDN allowlist for sc_id=%s",
                    sc_track_id,
                )
                return None
            logger.info("[SC-DL] Resolved to: %s", final_url[:120])
            return final_url
...
def _normalize_track_id(raw) -> str | None:
    ...
    if not s or s.lower() == "none":
        return None
    # Hard length cap before any parsing: a real SC track-id is ≤ 20 digits
    # and the historical tuple-blob bug topped out around ~400 chars. A
    # multi-KB string here is corrupt input — bail rather than feed it to
    # ast.literal_eval (worst-case quadratic on pathological literals).
    if len(s) > 2048:
        logger.warning(
            "[SC-DL] _normalize_track_id: oversized input (%d chars) — rejecting", len(s)
        )
        return None
...
def _download_hls_to_temp(
    ...
    # SSRF guard: m3u8 URL must already be a known SC CDN host. The signed
    # URL came from _resolve_stream_via_transcodings which fetched it from
    # SC's authenticated API — if SC ever pointed us elsewhere, we'd hand
    # the OAuth header to a stranger.
    if not _is_allowed_cdn_url(m3u8_url):
        logger.warning("[SC-DL] HLS m3u8 URL rejected by CDN allowlist")
        return None

    with tempfile.NamedTemporaryFile(prefix="rbpro_sc_", suffix=ext, delete=False) as tf:
        out_path = Path(tf.name)

    # ``-protocol_whitelist`` is critical: without it, ffmpeg's default
    # protocol list includes ``file:`` and ``concat:``. A hostile m3u8
    # could reference local files (read leak) or chain unexpected sources.
    # We pin to TLS/TCP only — exactly enough for an https HLS stream.
    # ``-fflags +nobuffer`` keeps memory bounded on long playlists.
    cmd = [
        FFMPEG_BIN,
        "-hide_banner",
        "-loglevel",
    ...
    # PRIVACY: only forward the OAuth token if the m3u8 endpoint is SC
    # first-party. Most HLS playlists live on cf-hls-media.sndcdn.com, which
    # IS first-party — but the rare AWS-backed playlist would otherwise have
    # received the user's bearer token in every segment fetch.
    # NOTE: where the token IS sent, it does land in the OS process listing
    # (ffmpeg accepts headers only as CLI args). Acceptable trade-off — a
    # local attacker who can read process args can already read the OS
    # keyring on the same machine. The alternative (Python-side segment
    # download + offline mux) doubles complexity for ~zero defensive benefit
    # on a single-user desktop app.
    if auth_token and _is_sc_first_party_url(m3u8_url):
        cmd += ["-headers", f"Authorization: OAuth {auth_token}\r\n"]
    cmd += ["-i", m3u8_url, "-c", "copy"]
    # Hard cap on muxed output size — defence against a malicious playlist
    # that loops segments forever to fill the disk. ffmpeg -fs aborts cleanly
    # once the output file would exceed the byte budget.
    cmd += ["-fs", str(_MAX_DOWNLOAD_BYTES)]
    if not is_mp3:
...
def _stream_file_to_temp(
    ...
    Returns None on any network error, allowlist rejection, or size violation.
    Partial temp files are unlinked before returning None — caller never sees
    them.
    """
    # SSRF guard: only stream from known SC CDN hosts. Belt-and-suspenders
    # vs. _resolve_official_download_url's check — direct callers in tests or
    # future code paths can't bypass it.
    if not _is_allowed_cdn_url(download_url):
        logger.warning("[SC-DL] download URL rejected by CDN allowlist")
        return None

    headers: dict[str, str] = {
        "User-Agent": _UA,
    }
    # PRIVACY: only attach the OAuth token when the URL is SC-owned. Signed
    # CDN URLs (S3 / Cloudfront / Akamai) carry their own short-lived query
    # signature and have no need for our bearer token — sending it there
    # leaks the user's credential into a third-party operator's access logs
    # AND can trigger 400/403 from AWS, which rejects unknown auth schemes.
    if auth_token and _is_sc_first_party_url(download_url):
        headers["Authorization"] = f"OAuth {auth_token}"
    ...
    try:
        resp = requests.get(
            ...
            allow_redirects=False,  # redirect target would re-need allowlist
        )
        resp.raise_for_status()

        # Early reject on advertised oversize — saves the whole transfer.
        content_length = resp.headers.get("Content-Length")
        if (
            content_length
            and content_length.isdigit()
            and int(content_length) > _MAX_DOWNLOAD_BYTES
        ):
            logger.warning(
                "[SC-DL] Content-Length %s exceeds limit %d — aborting download",
                content_length,
                _MAX_DOWNLOAD_BYTES,
            )
            resp.close()
        ...
        with tempfile.NamedTemporaryFile(prefix="rbpro_sc_", suffix=ext, delete=False) as tf:
            ...
            for chunk in resp.iter_content(chunk_size=65_536):
                if not chunk:
                    continue
                total += len(chunk)
                # Hard cap: bail mid-stream if the upstream keeps sending.
                # Protects against malicious / corrupted upstreams that
                # omit Content-Length or lie about it.
                if total > _MAX_DOWNLOAD_BYTES:
                    logger.warning(
                        "[SC-DL] Stream exceeded size limit (%d > %d) — aborting",
                        total,
                        _MAX_DOWNLOAD_BYTES,
                    )
                    resp.close()
