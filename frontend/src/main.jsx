...
const TAB_WORKSPACE = {
    ...
    'util-phrase': 'utilities',
    'util-duplicates': 'utilities',
    'util-xml': 'utilities',
    'util-converter': 'utilities',
    'util-low_quality': 'utilities',
    'util-lost': 'utilities',
    'util-no_artwork': 'utilities',
    insights: 'utilities',
};
            ...
            <div className="flex items-center gap-2 px-3.5 border-r border-line-subtle shrink-0">
                <div className="flex items-end gap-[2px]">
                    {[8, 12, 16, 11, 7].map((h, i) => (
                        <div
                            key={i}
                            className="bg-amber2 rounded-[1px]"
                ...
                <div className="flex flex-col leading-none gap-[3px]">
                    ...
                    {libraryStatus?.loaded && (
                        <span className="text-[9px] font-mono text-ink-muted">
                            {libraryStatus.tracks?.toLocaleString?.() ?? libraryStatus.tracks}{' '}
                            tracks
                        </span>
                    )}
                </div>
            ...
            <div className="relative z-10 flex flex-col items-center max-w-5xl w-full">
                <div className="flex items-end gap-1 mb-10">
                    {[28, 36, 44, 32, 20].map((h, i) => (
                        <div
                            key={i}
                            className="bg-amber2 rounded-[2px]"
                            style={{ width: 6, height: h }}
                        />
                    ))}
                </div>

                <h1 className="text-3xl font-bold text-ink-primary mb-3 tracking-tight">
                    XML Mode
                </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-4xl">
                    {cards.map(({ id, icon: Icon, title, desc }) => (
                        ...
                        >
                            ...
                            <h3 className="text-lg font-semibold text-ink-primary mb-1.5">
                                {title}
                            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
                ...
                >
                    ...
                    <p className="text-ink-secondary text-tiny leading-relaxed">
                        Direct integration with your <span className="font-mono">master.db</span>.
                        Fast, live edits — Rekordbox keeps its own backups.
                    </p>
                </button>
...
const App = () => {
    ...
    const handleXmlSubmode = useCallback(
        async (submode) => {
            try {
    ...
    // Reveal the Tauri window. Runs ONCE — it used to live in the effect
    // above, whose deps (appMode, libraryStatus.loaded) change during boot,
    // so every change ran the cleanup and cancelled the pending timer before
    // it could fire. The main window is configured `visible: false`, so a
    // cancelled timer means no GUI at all, with the backend running fine
    // behind it. Rust also reveals the window after 10s as a backstop
    // (spawn_window_watchdog), so a failure here degrades to a slow reveal
    // rather than an invisible app. No-op in the browser.
    useEffect(() => {
        if (!window.__TAURI__) return undefined;
        const splashTimer = setTimeout(() => {
            invoke('close_splashscreen').catch((err) =>
                console.error('[boot] close_splashscreen failed', err)
            );
        }, 2000);
    ...
    const handleTrackEdit = useCallback((track) => {
        setActiveTrack(track);
        setActiveTab('editor');
    }, []);
    ...
    return (
        <div className="flex flex-col h-screen w-screen bg-mx-deepest text-ink-primary overflow-hidden font-sans">
            ...
            {appMode === 'xml-submode' && (
                <XmlSubmodeView onPick={handleXmlSubmode} onBack={() => setAppMode('choice')} />
            )}
            ...
            {isInitialLoading && (
                <div className="fixed inset-0 z-[120] bg-mx-deepest flex flex-col items-center justify-center p-8 animate-fade-in font-sans">
                    ...
                    <div className="relative z-10 flex flex-col items-center max-w-md w-full">
                        ...
                        <h1 className="text-2xl font-semibold text-ink-primary mb-2 tracking-tight">
                            Loading Library
                        </h1>
                        <p className="mx-caption mb-10" style={{ color: 'var(--amber)' }}>
                        ...
                        <div className="w-full h-1 bg-line-subtle rounded-full overflow-hidden mb-3">
                            <div
                                ...
                                style={{
                                    width: libraryStatus.loaded
                                        ? '100%'
                                        : libraryStatus.tracks > 0
                                          ? '75%'
                                          : '20%',
                                }}
                            ></div>
                    ...
                    <Suspense fallback={<ViewLoader />}>
                        ...
                        <div className={activeTab === 'import' ? 'h-full' : 'hidden'}>
                            <ErrorBoundary key="eb-import">
                                <ImportView
                                    onSelectTrack={handleTrackEdit}
                                    onImportComplete={checkLibraryStatus}
                                    onPlayTrack={handlePlayTrack}
                                />
                            </ErrorBoundary>
                        </div>
                        ...
                        {activeTab === 'usb' && (
                            <ErrorBoundary key="eb-usb">
                                <UsbView />
                            </ErrorBoundary>
                        )}
                        {activeTab === 'usb-settings' && (
                            <ErrorBoundary key="eb-usb-settings">
                                <UsbSettingsView />
                            </ErrorBoundary>
                        )}
