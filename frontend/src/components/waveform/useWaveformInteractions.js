...
export default function useWaveformInteractions({
    ...
    const handleFileDrop = useCallback(
        (file) => {
            ...
            if (
                ...
                !/\.(wav|mp3|flac|aac|ogg|m4a)$/i.test(file.name)
            ) {
    ...
    // --- Hot Cues ---
    const handleSetHotCue = useCallback(
        (num) => {
    ...
    const handleJumpHotCue = useCallback(
        (num) => {
    ...
    const handleSetLoopIn = useCallback(
        () => setLoopIn(wavesurfer.current.getCurrentTime()),
        [setLoopIn, wavesurfer]
    );
    ...
    const handleGridShift = useCallback(
        (delta) => {
    ...
    const handleRender = useCallback(
        async (inputCuts, customName = null) => {
            ...
            try {
                const outputName =
                    customName ||
                    `${fullTrack.Title || track?.Title || 'Track'}_Edit_${Date.now()}.wav`;
                const payload = {
    ...
    useHotkeys('ctrl+shift+z, meta+shift+z, ctrl+y, meta+y', () => handleRedo(), editHotkeyOpts, [
        handleRedo,
    ]);
    ...
    // Zoom shortcuts
    useHotkeys(
        'ctrl+plus, meta+plus, ctrl+equal, meta+equal',
        () => setZoom((p) => Math.min(ZOOM_MAX, p + ZOOM_STEP)),
        editHotkeyOpts
    );
    useHotkeys(
        'ctrl+minus, meta+minus',
        () => setZoom((p) => Math.max(ZOOM_MIN, p - ZOOM_STEP)),
        editHotkeyOpts
    );
    // Loop in/out
    useHotkeys(
        'l',
        () => (loopIn === null ? handleSetLoopIn() : handleSetLoopOut()),
        editHotkeyOpts,
        [loopIn, handleSetLoopIn, handleSetLoopOut]
    );
    useHotkeys(
        'shift+l',
        () => {
