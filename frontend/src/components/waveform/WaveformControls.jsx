        ...
        <>
                ...
                <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md animate-fade-in">
                    <div className="w-80 p-8 glass-panel border border-amber2/20 rounded-2xl flex flex-col items-center">
                        ...
                        <p className="text-ink-secondary text-sm mb-6">
                            Processing insertion and effects...
                        </p>
                        ...
                        <div className="text-[10px] font-mono text-amber2 tracking-widest">
            ...
            <div className="rb-header">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3">
                        <span className="text-ink-muted font-bold tracking-tight">
                            RB EDITOR PRO
                        </span>
                    </div>
            ...
            <div className="h-10 bg-[#111] border-b border-white/5 px-4 flex items-center justify-between flex-shrink-0">
                ...
                <div className="flex items-center gap-4">
                        ...
                        {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => {
                            ...
                            return (
                                <button
                                    key={num}
                                    onClick={() =>
                                        cue ? jumpToCue(cue.InPoint) : handleSetHotCue(num)
                                    }
                                    onContextMenu={(e) => {
                                        e.preventDefault();
                    ...
                    {/* Volume Slider */}
                    <div
                        className="flex items-center bg-[#050505] h-8 px-2 rounded border border-white/5 gap-2"
                        title={`Volume: ${Math.round(internalVolume * 100)}%`}
                    >
                        <button
                        ...
                        />
                        <span className="text-[9px] font-mono text-ink-muted w-6 text-right">
                    ...
                    <div
                        ...
                        className={`flex items-center h-8 px-3 rounded border border-white/5 gap-2 cursor-pointer transition-all ${visualMode !== 'blue' ? 'bg-indigo-500/20 border-indigo-500/30' : 'bg-black'}`}
                    >
                        <Layers
                            size={12}
                            className={visualMode !== 'blue' ? 'text-indigo-400' : 'text-ink-muted'}
                        />
                        <span
                            className={`text-[10px] uppercase font-bold ${visualMode !== 'blue' ? 'text-indigo-400' : 'text-ink-muted'}`}
                        >
                            {visualMode}
                        </span>
                    </div>
                    ...
                    <div
                        ...
                        className={`flex items-center h-8 px-3 rounded border border-white/5 gap-2 cursor-pointer transition-all ${isQuantized ? 'bg-amber2/10 border-amber2/30' : 'bg-black'}`}
                    >
                        <RotateCcw
                            size={12}
                            className={isQuantized ? 'text-amber2' : 'text-orange-500'}
                        />
                        <span
                            className={`text-[10px] uppercase font-bold ${isQuantized ? 'text-amber2' : 'text-ink-muted'}`}
                        >
                            Q : {isQuantized ? 'ON' : 'AUTO'}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1 bg-[#050505] px-2 py-1 rounded border border-white/5">
                        ...
                        <button
                            onClick={() => handleGridShift(-0.01)}
                            className="p-1 hover:bg-white/5 rounded text-ink-secondary hover:text-white"
                            title="-10ms"
                        >
                            <ChevronLeft size={12} />
                        </button>
                        <button
                            ...
                            className="p-1 hover:bg-white/5 rounded text-amber2 hover:text-amber2"
                            title="Save Grid"
                        >
                            <Save size={12} />
                        </button>
            ...
            <div className="h-12 flex items-center justify-between px-6 bg-black border-b border-white/5">
                ...
                <div className="rb-metadata-box">
                    <span>
                        TIME{' '}
                        <span className="rb-metadata-value">
                            {fullTrack
                    ...
                    <span>
                        KEY <span className="rb-metadata-value">{fullTrack?.Key || '—'}</span>
                    </span>
                    <span>
                        BPM{' '}
                        <span className="rb-metadata-value">
                            {fullTrack && bpm ? bpm.toFixed(2) : '—'}
                        </span>
                    </span>
                </div>
            </div>
                    ...
                    <div className="rb-panel-title !bg-transparent !p-0 mb-3 flex items-center justify-between">
                        <span className="flex items-center gap-2">
                            <Infinity
                                size={12}
                                className={
                                    isLooping ? 'text-green-400 animate-pulse' : 'text-amber-500'
                                }
                            />{' '}
                            Loops
                        </span>
            ...
            <div className="w-1/2 flex flex-col p-2 bg-[#050505]">
                ...
                <div className="flex-1 border-b border-white/5 p-2 flex flex-col overflow-hidden">
                        ...
                        {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => {
                            ...
                            return (
                                <button
                                    key={num}
                                    onClick={() =>
                                        cue ? handleJumpHotCue(num) : handleSetHotCue(num)
                                    }
                                    onContextMenu={(e) => {
                                        e.preventDefault();
                                        if (cue) handleDeleteHotCue(num);
                                    ...
                                    title={
                                        cue
                                            ? `Jump to ${formatTime(cue.InPoint)} • Right-click to delete • Shift+${num} to overwrite`
                                            : `Set Hot Cue ${String.fromCharCode(64 + num)} (Shift+${num})`
                                    }
                                    className={`h-16 rounded border-2 flex flex-col items-center justify-center font-bold transition-all relative group ${
                                        cue
                                ...
                                >
                                    ...
                                    {cue && (
                                        <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 text-[8px] font-mono opacity-70">
                                            {formatTime(cue.InPoint)}
                                        </span>
                                    )}
                                    {!cue && (
                                        <Plus
                    ...
                    <button
                        ...
                        className="mt-2 h-7 bg-amber2/20 border border-amber2/30 text-amber2 text-[10px] font-bold rounded flex items-center justify-center gap-2"
                    >
                ...
                <div className="flex-1 p-2 flex flex-col justify-end">
                    <div className="rb-panel-title !bg-transparent !p-0 mb-3 text-ink-secondary uppercase tracking-tighter">
                        Edit Tools
                    </div>
                    <div className="grid grid-cols-4 gap-2">
                        <button
                            onClick={handleCopy}
                            className="rb-tool-btn !py-2"
                        ...
                        <button
                            onClick={handleClear}
                            className="rb-tool-btn !py-2 text-orange-400"
                        ...
                        <button
                            ...
                            className="rb-tool-btn !py-2 disabled:opacity-20"
                            title="Undo (Ctrl+Z)"
                        >
                            <RotateCcw size={14} />
                            UNDO
                        ...
                        <button
                            onClick={() => handleRender(cuts)}
                            className="rb-tool-btn !py-2 text-amber2 border-amber2/10"
                            title="Export edited audio (Ctrl+E)"
                        >
                            <Download size={14} />
                            EXPORT
                        </button>
