        ...
        <div className="absolute top-2 right-2 bg-black/85 backdrop-blur-md border border-white/10 rounded-lg p-2 text-[10px] font-mono text-ink-secondary w-[240px] max-h-[60%] overflow-y-auto">
            <div className="text-amber-400 font-bold mb-1.5 flex items-center justify-between">
                <span className="flex items-center gap-1">
                    <Scissors size={10} /> {cuts.length} Edit{cuts.length === 1 ? '' : 's'}
                </span>
                <button
                    onClick={handleClear}
            ...
            {cuts.map((c) => {
                ...
                return (
                    <div
                        ...
                        onClick={() => {
                            wavesurfer.current?.setTime(jumpTime);
                            setCurrentTime(jumpTime);
                        }}
                        title="Click to jump to this edit"
                    >
                        <span
                            className={`truncate flex-1 ${isDelete ? 'text-red-400' : 'text-green-400'}`}
                        >
                            {isDelete
                                ? `DEL ${c.start.toFixed(1)}–${c.end.toFixed(1)}s`
                                : `INS @${(c.insertAt ?? 0).toFixed(1)}s`}
                        </span>
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
