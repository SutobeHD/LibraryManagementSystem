...
const PlaylistNode = ({
    ...
    const handleDrop = (e) => {
        ...
        try {
            ...
            const position =
                dragOver === 'top' ? 'before' : dragOver === 'bottom' ? 'after' : 'inside';
    ...
    return (
        <div data-pl-row={node.ID}>
            ...
            >
                {dragOver === 'top' && (
                    <div className="absolute top-0 left-0 right-0 h-0.5 bg-amber2 z-10" />
                )}
                {dragOver === 'bottom' && (
                    <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-amber2 z-10" />
                )}
                ...
                <span
                    className={`text-[13px] truncate flex-1 ${isIntelligent ? 'text-purple-300' : ''} ${isSelected ? 'font-semibold' : ''}`}
                >
                    {node.Name}
                </span>

                {isIntelligent && (
                    <span className="text-[9px] text-purple-500/60 font-bold uppercase tracking-wider shrink-0">
                        Smart
                    </span>
                )}
...
const PlaylistBrowser = ({ onSelectTrack, onEditTrack, onPlayTrack, libraryStatus }) => {
    ...
    const handleCreatePlaylist = async () => {
        ...
        try {
            const parentId =
                contextMenu?.node?.Type === '0'
                    ? contextMenu.node.ID
                    : selectedPlaylist?.Type === '0'
                      ? selectedPlaylist.ID
                      : 'ROOT';
            ...
            if (node.Type === '4') {
                ...
                if (newId && node.Type === '1') {
                    ...
                    for (const t of tracks.data || []) {
                        const tid = t.id || t.ID;
                        if (tid)
                            await api.post('/api/playlists/add-track', {
                                pid: newId,
                                track_id: String(tid),
                            });
                    }
    ...
    const handleRemoveTrack = async (trackId) => {
        ...
        try {
            await api.post('/api/playlists/remove-track', {
        ...
        for (const tok of tokens) {
            if (tok.includes(':')) {
                ...
                if (field === 'bpm') {
                    ...
                    if (val.startsWith('>')) {
                        if (!(bpm > parseFloat(val.slice(1)))) return false;
                        continue;
                    }
                    if (val.startsWith('<')) {
                        if (!(bpm < parseFloat(val.slice(1)))) return false;
                    ...
                    if (Math.round(bpm) !== Math.round(parseFloat(val))) return false;
                    continue;
                }
                if (field === 'key') {
                    if (
                ...
                if (field === 'year') {
                    if (String(t.ReleaseYear || '') !== val) return false;
                    continue;
                }
                if (field === 'rating') {
                    const r = parseInt(t.Rating || 0);
                    if (val.startsWith('>')) {
                        if (!(r > parseInt(val.slice(1)))) return false;
                        continue;
                    }
                    if (val.startsWith('<')) {
                        if (!(r < parseInt(val.slice(1)))) return false;
                    ...
                    if (r !== parseInt(val)) return false;
                    continue;
                }
            ...
            <div className="flex h-full flex-col items-center justify-center text-ink-muted bg-mx-deepest/50">
                ...
                <div className="flex flex-col items-center">
                    <p className="max-w-md text-center mb-8">
                        No playlists found. Please try refreshing or check your database connection.
                    </p>
                    <button
                        onClick={loadTree}
    ...
    return (
        <div className="flex h-full relative">
            ...
            >
                ...
                <div className="sticky top-0 bg-mx-deepest/90 z-10 border-b border-white/5 backdrop-blur-xl shrink-0">
                    <div className="flex justify-between items-center px-3 py-3">
                        <h3 className="text-[10px] font-bold text-ink-muted uppercase tracking-widest">
                            Library
                        </h3>
                        <div className="flex gap-0.5">
                            ...
                            <button
                                ...
                                className="p-1.5 hover:bg-white/5 rounded text-amber2"
                                title="New Playlist"
                            >
                                <Plus size={12} />
                            </button>
                            <button
                                ...
                                className="p-1.5 hover:bg-white/5 rounded text-amber-500"
                                title="New Folder"
                            >
                                <FolderPlus size={12} />
                            </button>
                ...
                >
                    <div
                        className={`p-1.5 rounded-lg ${showCollection ? 'bg-amber2/20' : 'bg-white/5'}`}
                    >
                ...
                <div className="px-1 pt-2 pb-1">
                    <div className="text-[9px] font-bold text-ink-placeholder uppercase tracking-widest px-2 mb-1">
                        Playlists
                    </div>
                </div>
                <div
                        ...
                        >
                            ...
                            {(contextMenu.node.Type === '1' || contextMenu.node.Type === '4') && (
                                <button
                                    ...
                                    className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-ink-primary"
                                >
                            ...
                            {contextMenu.node.Type === '4' && (
                                <button
                                    onClick={() => {
                                        setSmartEditorEditing({
                                            id: contextMenu.node.ID,
                                            name: contextMenu.node.Name,
                                            criteria: contextMenu.node.SmartList || {},
                                        });
                                        setShowSmartEditor(true);
                                        setContextMenu(null);
                                    }}
                                    className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-purple-300"
                                >
                            ...
                            <button
                                ...
                                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-cyan-300"
                            >
                                <Plus size={14} /> Duplizieren
                            </button>
                            <button
                                onClick={() => handleDelete(contextMenu.node)}
                                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-red-400"
                            >
                            ...
                            <button
                                onClick={handleCreateFolder}
                                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-amber-500"
                            >
                                <FolderPlus size={14} /> Neuer Ordner
                            </button>
                            <button
                                onClick={handleCreatePlaylist}
                                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-amber2"
                            >
                                <Plus size={14} /> Neue Playlist
                            </button>
                            <button
                                onClick={() => {
                                    setSmartEditorParent(
                                        contextMenu?.node?.Type === '0'
                                            ? contextMenu.node.ID
                                            : 'ROOT'
                                    );
                                ...
                                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-sm transition-colors text-purple-400"
                            >
            ...
            <div className="flex-1 bg-transparent flex flex-col overflow-hidden relative">
                {selectedPlaylist || showCollection ? (
                    <div className="flex-1 overflow-y-auto flex flex-col">
                        ...
                        <div className="p-5 border-b border-white/5 bg-mx-shell/40 backdrop-blur-xl flex justify-between items-end shrink-0">
                            <div className="flex items-center gap-4">
                                <div
                                    className={`p-2.5 rounded-xl ${showCollection ? 'bg-amber2/15' : isIntelligentSelected ? 'bg-purple-500/15' : 'bg-mx-card'}`}
                                >
                                    {showCollection ? (
                                        <Database size={24} className="text-amber2" />
                                    ) : isIntelligentSelected ? (
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                                        {currentViewTitle}
                                        {isIntelligentSelected && (
                                            <span className="text-xs bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">
                                                Intelligent
                                            </span>
                                        )}
                                    </h1>
                                    <p className="text-ink-muted font-mono text-xs mt-0.5">
                                        {currentViewTracks.length}
                                        {currentViewTracks.length !== currentViewCount
                                            ? ` / ${currentViewCount}`
                                            : ''}{' '}
                                        TRACKS
                                    </p>
                                ...
                                {selectedPlaylist && !isIntelligentSelected && (
                                    <>
                                        ...
                                        >
                                            <Upload
                                                size={12}
                                                className={scExporting ? 'animate-pulse' : ''}
                                            />
                            ...
                            <div className="flex-1 overflow-hidden flex flex-col p-3 pt-1">
                                <TrackTable
                                    ...
                                    onReorder={
                                        selectedPlaylist && !isIntelligentSelected
                                            ? (trackId, newIndex) => {
                                                  api.post('/api/playlists/reorder', {
                                                      pid: selectedPlaylist.ID,
                                                      track_id: trackId,
                                    ...
                                    playlistId={
                                        showCollection ? 'COLLECTION' : selectedPlaylist?.ID
                                    }
                                />
                            </div>
                        )}
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-ink-muted">
                        ...
                        <p className="text-xl font-light text-ink-muted">
            ...
            <SoundCloudProgressModal
                isOpen={showScProgress}
                onClose={() => setShowScProgress(false)}
            />
        </div>
    );
};
