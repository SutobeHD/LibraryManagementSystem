        ...
        <div className="w-full h-full flex flex-col bg-mx-deepest/80 border-t border-white/5">
            ...
            <div className="flex items-center justify-between px-2 py-1 border-b border-white/5 bg-mx-shell/50 h-[32px] shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div
                        className={`flex items-center gap-2 bg-mx-card/50 rounded px-2 py-0.5 border border-white/5 w-48 transition-opacity ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
                    >
            ...
            <div
                className={`flex-1 overflow-hidden flex flex-col ${isCollapsed ? 'invisible' : 'visible'}`}
            >
                ...
                {activeSection === 'projects' && (
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                            {projects.length === 0 ? (
                                <p className="col-span-full text-xs text-ink-placeholder text-center py-8">
                                    No recent projects
                                </p>
                            ) : (
...
const TabButton = ({ icon, label, active, onClick }) => (
    <button
        onClick={onClick}
        className={`flex items-center px-3 py-1 text-[10px] uppercase font-bold rounded-t-lg transition-colors border-b-2 ${
            active
