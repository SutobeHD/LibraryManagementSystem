...
import { Save, FolderOpen, Download, FileAudio, Hash } from 'lucide-react';

const DawToolbar = React.memo(({ state, dispatch, onSave, onOpen, onExport, onAutoCue }) => {
        ...
        <div className="h-11 bg-mx-shell/60 border-b border-white/5 flex items-center px-4 gap-3 shrink-0 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-1.5 mr-4">
                <div className="flex flex-col items-center px-2.5 py-0.5 bg-mx-card border border-white/10 rounded-sm">
                    <span className="text-[8px] text-ink-muted uppercase tracking-[0.12em] font-bold">
                        BPM
                    </span>
                    <span className="text-[13px] text-amber2 font-mono font-bold leading-none">
                        {bpm?.toFixed(1) || '---'}
                    </span>
                </div>
                <div className="flex flex-col items-center px-2.5 py-0.5 bg-mx-card border border-white/10 rounded-sm">
                    ...
                    <span
                        className={`text-[13px] font-mono font-bold leading-none ${snapEnabled ? 'text-amber2' : 'text-ink-placeholder'}`}
                    >
                        {snapEnabled ? 'ON' : 'OFF'}
                    </span>
                </div>
            ...
            <div className="flex items-center gap-1">
                <ToolBtn
                    icon={<Hash size={14} />}
                    label="Auto-Generate 16-Bar Markers"
                    onClick={onAutoCue}
                />
            </div>
            ...
            {state.trackMeta.title && (
                ...
                >
                    <div className="truncate text-xs">
                        <span className="text-ink-secondary">{state.trackMeta.artist}</span>
...
const ToolBtn = React.memo(({ icon, label, onClick, disabled, accent, danger }) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`h-7 w-7 flex items-center justify-center rounded-sm border transition-all ${
            disabled
