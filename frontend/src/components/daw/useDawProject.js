...
export default function useDawProject({ state, dispatch, setActiveTrack }) {
        ...
        async (e) => {
            ...
            reader.onload = async (ev) => {
                ...
                try {
                    ...
                    const editTimelineEnd =
                        trackData.regions && trackData.regions.length > 0
                            ? trackData.regions.reduce(
                                  (max, r) => Math.max(max, r.timelineEnd || 0),
                                  0
                              )
                    ...
                    const projectName =
                        project.info?.name || file.name.replace('.rbep', '') || 'Untitled Project';
                    ...
                    try {
                        ...
                        console.warn(
                            '[DjEditDaw] Band peaks failed during open, using fallback:',
                            err
                        );
                    }
                    ...
                    newState.tempoMap =
                        project.mastergrid?.beats?.length > 0
                            ? project.mastergrid.beats
                            : trackData.songTempoMap?.length > 0
                              ? trackData.songTempoMap
                              : buildTempoMap(
                                    newState.project.bpm,
                                    0,
                                    newState.totalDuration * 1000
                                );
                    ...
                    const { hotCues, memoryCues, loops } = cuePointsToState(
                        trackData.cuePoints || []
                    );
                    ...
                    if (trackData.volume && trackData.volume.length > 0) {
                        ...
                        newState.volumeData = trackData.volume.map((v) => ({
                            startSec:
                                timelineTempoMap.length > 0
                                    ? (timelineTempoMap[
                                          Math.min(
                                              Math.floor(v.startBeat),
                                              timelineTempoMap.length - 1
                                          )
                                      ]?.positionMs || 0) / 1000
                    ...
                    if (err.message.includes('fetch')) {
                        ...
                        toast.error(
                            `Failed to load: ${err.message} (${audioPath || 'Unknown Path'})`,
                            { id: 'daw-open', duration: TOAST_DURATION_LONG_MS }
                        );
                    } else {
    ...
    const handleSave = useCallback(async () => {
        try {
            ...
            dispatch({
                type: 'SET_PROJECT',
                payload: { name: projectName, filepath, dirty: false },
            });
