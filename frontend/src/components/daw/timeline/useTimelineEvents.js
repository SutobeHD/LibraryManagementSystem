    ...
    const handleMouseDown = useCallback(
        (e) => {
            ...
            if (e.button === 0) {
                ...
                if (clickedRegion) {
                    dispatch({
                        type: e.ctrlKey ? 'TOGGLE_SELECT_REGION' : 'SELECT_REGION',
                        payload: clickedRegion.id,
                    });
                } else {
                    if (!e.ctrlKey) {
                        dispatch({
                            type: 'SET_SELECTION_RANGE',
                            payload: { start: time, end: time },
                        });
    ...
    const handleWheel = useCallback(
        (e) => {
            ...
            if (e.ctrlKey || e.metaKey) {
                ...
                dispatch({
                    type: 'SET_SCROLL_X',
                    payload: Math.max(0, timeBefore * newZoom - mouseX),
                });
            } else {
                dispatch({
                    type: 'SET_SCROLL_X',
                    payload: Math.max(0, d.scrollX + (e.deltaY || e.deltaX) * 2),
                });
            }
        },
        [canvasRef, ds, dispatch]
