/**
 * useTimelineRender — Rendering layer for DawTimeline
 *
 * Owns:
 *  - State-sync effect (React state → mutable ds.current)
 *  - RAF loop (auto-follow scroll, LOD hysteresis, bitmap rebuild, draw)
 *  - OffscreenCanvas waveform bitmap cache + invalidation key
 *  - All draw functions (grid, regions, cues, loops, playhead, ruler, phase meter)
 *  - All band-specific waveform renderers
 *
 * Layered canvas model:
 *  Layer 0 [CACHED]  — Static waveform bitmap (OffscreenCanvas)
 *  Layer 1 [LIVE]    — Grid, cues, loops, region boundaries, selection
 *  Layer 2 [60FPS]   — Playhead + phase meter
 */
...
export const PHASE_METER_HEIGHT = 8;
...
/**
 * Drives the canvas rendering loop and keeps the mutable draw-state ref in
 * sync with the React state.
 *
 * @param {Object}   args
 * @param {Object}   args.state          DAW state
 ...
 * @param {Object}   args.goodFramesRef  ref counting good frames for LOD hysteresis
 */
export function useTimelineRender({ state, dispatch, canvasRef, ds, goodFramesRef }) {
    ...
    useEffect(() => {
        ...
        d.sourceDuration =
            state.sourceBuffer?.duration || state.trackMeta?.duration || state.totalDuration;
        ...
        if (state.regions?.length) {
            ...
            for (const r of state.regions) {
                const sample =
                    ((r.timelineStart || 0) + (r.duration || 0) + (r.sourceStart || 0)) * 1000;
    ...
    useEffect(() => {
        ...
        const frame = (ts) => {
            ...
            if (d.isPlaying) {
                ...
                if (d.scrollSyncAccum >= 12) {
                    // ~5 dispatches/sec at 60fps
                    d.scrollSyncAccum = 0;
...
function buildWaveformBitmap(d) {
    ...
    for (const region of regions) {
        ...
        if (waveformStyle === 'mono' && hasMono) {
            ...
            hasBand &&
            activePeaks.low?.length &&
            activePeaks.mid?.length &&
            activePeaks.high?.length
        ) {
...
function drawMixxxFilteredWaveform(
    ctx,
    bandPeaks,
    region,
    rStartPx,
...
function drawSmoothBandPath(
    ...
    grad.addColorStop(
        0.45,
        c.top.includes('0.92') ? c.top.replace('0.92', '0.60') : c.top.replace('0.88', '0.55')
    );
    ...
    for (let t = first; t <= endTime + gridUnit; t += gridUnit) {
        ...
        ctx.strokeStyle =
            beatNum % 4 === 0
                ? COLORS.gridBar
                : ppb > 60
                  ? Math.round((t - firstBeatSec) / (beatDur / 4)) % 4 === 0
                      ? COLORS.gridBeat
                      : COLORS.gridSub
                  : COLORS.gridBeat;
...
function drawRegionBoundaries(ctx, d, startTime, endTime) {
    ...
    for (const region of regions) {
        ...
        ctx.lineTo(Math.round(x1) + 0.5, waveBot);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(Math.round(x2) + 0.5, RULER_HEIGHT);
        ctx.lineTo(Math.round(x2) + 0.5, waveBot);
        ctx.stroke();
    }
}
...
function drawCueMarkers(ctx, d, startTime, endTime) {
    ...
    if (ghostCueX !== null) {
        ...
        ctx.setLineDash([4, 4]);
        ctx.beginPath();
        ctx.moveTo(ghostCueX + 0.5, RULER_HEIGHT);
        ctx.lineTo(ghostCueX + 0.5, waveBot);
        ctx.stroke();
        ctx.setLineDash([]);
    }
}
...
function drawLoops(ctx, d, startTime, endTime) {
    ...
    for (let i = 0; i < loops.length; i++) {
        ...
        ctx.fillStyle = active
            ? `rgba(${loop.red ?? 251},${loop.green ?? 146},${loop.blue ?? 60},0.14)`
            : COLORS.loopFill;
        ctx.fillRect(x1, RULER_HEIGHT, x2 - x1, waveBot - RULER_HEIGHT);
