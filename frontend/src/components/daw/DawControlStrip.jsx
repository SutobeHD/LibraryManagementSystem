...
const DawControlStrip = React.memo(
    ({ state, dispatch, onPlay, onStop, onSplit, onRippleDelete, onJumpTo, onExport }) => {
        const {
            isPlaying,
            playhead,
            bpm,
        ...
        const toggleTimeMode = useCallback(
            () => setTimeMode((m) => (m === 'elapsed' ? 'remaining' : 'elapsed')),
            []
        );
        ...
        const handleDeleteHotCue = useCallback(
            (index, e) => {
                e.stopPropagation();
                dispatch({
                    type: 'PUSH_UNDO',
                    payload: `Delete Hot Cue ${String.fromCharCode(65 + index)}`,
                });
        ...
        const handleLoopIn = useCallback(() => {
            ...
            if (loops.length === 0 || loops[loops.length - 1].endTime != null) {
                ...
                dispatch({
                    type: 'UPDATE_LOOP',
                    payload: { index: loops.length - 1, updates: { startTime: time } },
                });
            }
                ...
                if (lastLoop.endTime == null && time > lastLoop.startTime) {
                    dispatch({
                        ...
                        payload: {
                            ...
                            updates: { endTime: time, active: true },
                        },
                    });
        ...
        const handleShiftGrid = useCallback(
            (delta) => {
        ...
        const handleAdjustBpm = useCallback(
            (delta) => {
            ...
            <div className="bg-mx-shell/80 border-t border-white/5 flex flex-col shrink-0 overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-3 py-2 border-b border-white/5">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button
                            ...
                            style={
                                isPlaying ? { boxShadow: '0 0 12px var(--amber-glow)' } : undefined
                            }
                    ...
                    <div className="flex items-stretch gap-1.5 shrink-0">
                        ...
                        >
                            <span
                                className={`text-[13px] font-mono font-bold leading-none ${timeMode === 'remaining' ? 'text-amber2-hover' : 'text-white'}`}
                            >
                                {timeMode === 'remaining'
                                    ? formatRemaining(playhead)
                                    : formatTime(playhead)}
                            </span>
                        ...
                        <div className="flex flex-col items-center justify-center px-2 py-1 bg-mx-card border border-white/10 rounded-sm">
                            <div className="flex items-center gap-1">
                                <button
                                    ...
                                    className="text-ink-placeholder hover:text-white"
                                >
                                    <Minus size={9} />
                                </button>
                                <span className="text-[13px] font-mono font-bold text-amber2 leading-none">
                                    {bpm?.toFixed(1) || '---'}
                                </span>
                                <button
                                    onClick={() => handleAdjustBpm(0.1)}
                            ...
                            <span className="text-[8px] text-ink-muted uppercase tracking-wider mt-1">
                                BPM
                            </span>
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-2 px-3 py-1.5">
                    ...
                    <div className="flex items-center gap-0.5">
                        ...
                        <span className="text-[8px] text-ink-muted font-bold uppercase tracking-wider px-1 text-center leading-tight">
                            Grid
                            <br />
                            Shift
                        </span>
                    ...
                    <div className="flex items-center gap-0.5">
                        ...
                        <TBtn
                            onClick={() => dispatch({ type: 'PASTE_INSERT' })}
                            disabled={state.clipboard?.length === 0}
                            title="Paste Insert (Ctrl+V)"
                        >
                            <Clipboard size={14} />
                        </TBtn>
                        <TBtn
                    ...
                    <div className="flex items-center gap-1.5">
                        <button
                            ...
                            className={`flex items-center gap-1 h-7 px-2 rounded-sm border text-[10px] font-bold uppercase tracking-wider transition-all ${
                                snapEnabled
                        ...
                        />
                        <span className="text-[9px] text-ink-muted font-mono w-7">
                            {Math.round(zoom)}
                        </span>
                        <TBtn
                    ...
                    <div className="flex items-center gap-0.5">
                        ...
                        {loops.length > 0 && activeLoopIndex >= 0 && (
                            <TBtn
                                onClick={() =>
                                    dispatch({ type: 'REMOVE_LOOP', payload: activeLoopIndex })
                                }
                                danger
                                title="Delete Loop"
                            >
...
const TBtn = React.memo(({ onClick, children, disabled, active, danger, title }) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`h-7 w-7 flex items-center justify-center rounded-sm border transition-colors ${
            disabled
