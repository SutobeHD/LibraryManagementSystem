...
const ImportView = ({ onSelectTrack, onImportComplete }) => {
    ...
    useEffect(() => {
        api.get('/api/library/quality-stats')
            .then((res) => {
        ...
        async (paths, opts = {}) => {
            ...
            try {
                ...
                if (queued_total > 0) {
                    ...
                    toast.success(
                        message || `Queued ${queued_total} audio file(s) for import${plBadge}`
                    );
    ...
    const browseFolder = useCallback(async () => {
        try {
            ...
            toast.error(
                'Folder picker unavailable in browser mode — drop a folder or use Browse Files.'
            );
        }
    }, [importPaths]);
            ...
            setFiles((prev) =>
                prev.map((row) => {
                    ...
                    return {
                        ...
                        totalTime:
                            typeof task.total_time === 'number' ? task.total_time : row.totalTime,
    ...
    const losslessPct =
        qualityStats.total > 0 ? Math.round((qualityStats.lossless / qualityStats.total) * 100) : 0;
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-tiny text-ink-muted">
                                Drag & drop audio files to analyze and add to your library
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col p-6 gap-5">
                    ...
                    >
                        <Upload
                            size={28}
                            className={isDragging ? 'text-amber2 mb-2' : 'text-ink-muted mb-2'}
                        />
                        <p className="text-[13px] font-medium text-ink-primary">
                            Drop audio files or whole folders here
                        </p>
                        <p className="text-[11px] text-ink-muted mt-1">
                            All audio formats supported · Folders are scanned recursively
                        </p>
                        <input
                            type="file"
                            multiple
                            ...
                            onChange={(e) =>
                                handleFiles(Array.from(e.target.files).filter(isAudioFile))
                            }
                            className="hidden"
                            id="fileInput"
                        />
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden">
                            ...
                            <div className="flex-1 overflow-y-auto pt-1">
                                {files.map((f) => (
                                    <div
                                        key={f.id}
                                        className="p-2.5 border-b border-line-subtle flex items-center gap-3 group hover:bg-mx-hover transition-colors"
                                    >
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {f.status === 'success' && (
                                                <div className="flex items-center gap-2">
                                                    <div className="text-right">
                                                        <span className="text-[9px] font-semibold text-ok uppercase">
                                                            Done
                                                        </span>
                                                        {f.bpm && (
                                                            <span className="text-[10px] font-mono text-amber2 ml-2">
                                                                {f.bpm.toFixed(1)} BPM
                                                            </span>
                                                        )}
                                                    </div>
                                                    ...
                                                    <button
                                                        onClick={() =>
                                                            onSelectTrack({ id: f.trackId })
                                                        }
                                                        className="p-1.5 bg-ok/10 hover:bg-ok/20 rounded-mx-xs text-ok"
                                                    >
                                                        <Scissors size={12} />
                                                    </button>
                                                </div>
                                            ...
                                            {f.status === 'error' && (
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[9px] font-semibold text-bad">
                                                        {f.error}
                                                    </span>
                                            ...
                                            {f.status === 'pending' && !importing && (
                                                <button
                                                    ...
                                                    className="p-1.5 text-ink-placeholder hover:text-bad opacity-0 group-hover:opacity-100 transition-opacity"
                                                >
                                                    <Trash2 size={13} />
                                                </button>
                                            )}
            ...
            <div className="w-80 border-l border-line-subtle bg-mx-shell overflow-y-auto p-4 space-y-4 shrink-0">
                ...
                <div className="mx-card p-4">
                    ...
                    <div className="flex justify-between text-[10px]">
                        <span className="flex items-center gap-1">
                            ...
                            <span className="text-ink-secondary">
                                Lossless: {qualityStats.lossless}
                            </span>
                        </span>
                        <span className="flex items-center gap-1">
                    ...
                    <p className="text-[9px] text-ink-muted mt-2">
                        WAV, FLAC, AIFF = Lossless · MP3, AAC, M4A, OGG = Lossy
                    </p>
                </div>
                ...
                <div className="mx-card p-4">
                    ...
                    {conversionEnabled && (
                        <div className="space-y-2.5 pt-1 border-t border-line-subtle">
                            <div>
                                <label className="text-[10px] text-ink-muted uppercase tracking-wider font-semibold block mb-1">
                                    Target Format
                                </label>
                                <select
                                    value={targetFormat}
                ...
                <div className="mx-card p-4">
                    ...
                    <label className="flex items-center justify-between p-2 rounded-mx-sm hover:bg-mx-hover cursor-pointer transition-all">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-ink-muted">
                                Files are validated before originals are removed
                            </span>
                        </div>
                        <input
                            type="checkbox"
                            checked={backupOriginals}
                            onChange={(e) => setBackupOriginals(e.target.checked)}
                            className="accent-amber2 w-3.5 h-3.5"
                        />
                    </label>
                    <div className="mt-2 p-2 bg-mx-input rounded-mx-xs border border-line-subtle flex items-center gap-2">
                        ...
                        <span className="text-[9px] text-ink-muted font-mono truncate">
                            ~/Music/RB_Backups/originals/
                        </span>
                    </div>
                </div>
            </div>
