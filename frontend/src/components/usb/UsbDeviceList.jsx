        ...
        <div className="w-72 border-r border-line-subtle overflow-y-auto p-2 space-y-1 bg-mx-shell">
            ...
            {allDevices.length === 0 && !scanning && (
                <div className="flex flex-col items-center justify-center h-48 text-center px-4">
                    ...
                    <p className="text-[10px] text-ink-placeholder mt-1">
                        Insert a USB stick and click Scan
                    </p>
                    {hiddenCount > 0 && (
                        <p className="text-[10px] text-ink-placeholder mt-2">
                            {hiddenCount} system drive(s) hidden
                        </p>
                    )}
                </div>
            )}
                ...
                <div className="flex flex-col items-center justify-center h-32 gap-3 px-4 text-center">
                    ...
                    {slowScan && (
                        <p className="text-[10px] text-ink-placeholder">
                            Still scanning — a disconnected or unreadable drive can slow this. Use
                            Scan to retry.
                        </p>
                    )}
                </div>
            ...
            {allDevices.map((device) => {
                ...
                const usedPct =
                    totalSpace > 0 ? Math.max(0, Math.min(100, (usedSpace / totalSpace) * 100)) : 0;
                const usedColor =
                    usedPct > 95 ? 'var(--bad)' : usedPct > 80 ? 'var(--amber)' : '#2DD4BF';
                return (
                    <button
                        ...
                        <div className="flex items-center gap-2.5">
                            <div
                                className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                                    connected
                                        ? 'bg-ok shadow-[0_0_4px_#3DD68C]'
                                        : 'bg-ink-placeholder'
                                }`}
                            />
                            <HardDrive
                                size={14}
                                className={isSelected ? 'text-amber2' : 'text-ink-secondary'}
                            />
                            ...
                            {worst === 'partial' && (
                                <AlertTriangle
                                    ...
                                    title="Wrong format for CDJs — consider reformatting"
                                />
                            )}
                            {worst === 'warn' && (
                                <AlertTriangle
