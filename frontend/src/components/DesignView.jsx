...
const DesignView = () => {
    ...
    const concepts = {
        ...
        spectral: { id: 'spectral', name: 'Spectral Prism', description: 'Technical Analytical. Clean data-heavy look.', color: 'from-slate-400 to-slate-200', accent: 'text-cyan-400', bg: 'bg-[#131b26]', panel: 'bg-[#1c2633] border-slate-700', btn: 'bg-slate-700 hover:bg-slate-600 border border-slate-500 text-white', layout: 'standard' },
        liquid: { id: 'liquid', name: 'Liquid Sound', description: 'Glassmorphism Aqua. Translucent layers and heavy blur.', color: 'from-cyan-400 to-blue-400', accent: 'text-cyan-300', bg: 'bg-slate-950', panel: 'bg-white/5 backdrop-blur-2xl border-white/10 shadow-xl', btn: 'bg-cyan-500/80 hover:bg-cyan-400 backdrop-blur-md border border-white/20', layout: 'standard' },
        ...
        stream: { id: 'stream', name: 'Horizontal Stream', description: 'Cinematic Flow. Focus on full-width waveforms.', color: 'from-cyan-500 to-blue-600', accent: 'text-cyan-400', bg: 'bg-slate-950', panel: 'bg-white/5 backdrop-blur-xl border-y border-white/5', btn: 'bg-transparent border border-cyan-500 text-cyan-500 hover:bg-cyan-500 hover:text-white', layout: 'stream' },
        ...
        minimal: { id: 'minimal', name: 'Streamline List', description: 'Distraction Free. Modern, high-whitespace minimalist list.', color: 'from-slate-200 to-slate-400', accent: 'text-blue-600', bg: 'bg-white text-slate-900', panel: 'bg-slate-50 border-slate-200', btn: 'bg-slate-900 text-white hover:bg-slate-800 rounded-lg', layout: 'lib_minimal' },
        sonic_list: { id: 'sonic_list', name: 'Sonic Row', description: 'Visual Audio. Every track row includes an integrated waveform.', color: 'from-cyan-400 to-emerald-400', accent: 'text-emerald-500', bg: 'bg-[#050c14]', panel: 'bg-[#0a1622] border-emerald-500/10', btn: 'bg-emerald-600 hover:bg-emerald-500', layout: 'lib_waveform' },
            ...
            <div className="space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className={`p-8 rounded-3xl border ${concept.panel}`}>
                        <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-6">Component Preview</h3>
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase"><span>Analyzing Audio</span><span>75%</span></div>
    ...
    return (
        <div className={`flex h-full w-full ${concept.bg.includes('white') ? 'bg-slate-50 text-slate-900' : 'bg-slate-950 text-white'} overflow-hidden animate-fade-in font-sans`}>
            {/* Design Sidebar */}
            <div className={`w-72 border-r ${concept.bg.includes('white') ? 'border-slate-200 bg-white' : 'border-white/5 bg-black/40'} p-6 flex flex-col gap-6 shrink-0 h-full overflow-y-auto`}>
                <div>
                    ...
                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest italic">Phase 3: Library Evolution</p>
                </div>
                ...
                <div className="max-w-5xl mx-auto flex flex-col gap-12">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="mt-4 text-slate-500 text-lg max-w-2xl font-bold italic">{concept.description}</p>
                        </div>
            ...
            {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className={`flex items-center gap-6 p-4 rounded-xl transition-all cursor-pointer ${i === 1 ? 'bg-slate-50 shadow-sm' : 'hover:bg-slate-50/50 grayscale opacity-60 hover:grayscale-0 hover:opacity-100'}`}>
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs font-medium text-slate-400">Streamline Artist</p>
                    </div>
                    <div className="text-right">
                        <p className="font-mono text-sm font-bold text-blue-600">6A</p>
                        <p className="text-[10px] font-black text-slate-400">122 BPM</p>
                    </div>
        ...
        {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className={`group bg-[#0a1622] rounded-2xl border border-emerald-500/10 p-4 flex items-center gap-6 hover:border-emerald-500/40 transition-all`}>
                ...
                <div className="w-48">
                    ...
                    <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Oscillator X</p>
                </div>
                ...
                <div className="w-20 text-right shrink-0">
                    ...
                    <p className="text-[9px] font-black text-slate-500">4:25</p>
                </div>
            </div>
        ))}
    </div>
    ...
    <div className="columns-2 gap-8">
        {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="mb-8 p-10 bg-white/5 border border-purple-500/10 rounded-[40px] break-inside-avoid group hover:border-purple-500/50 transition-all">
                ...
                <p className="text-slate-500 text-sm font-bold leading-relaxed mb-8">A deep exploration of minimal textures and syncopated rhythms from the Berlin underground.</p>
            ...
            {[1, 2].map(i => (
                <div key={i} className="p-4 flex flex-col justify-between border-r border-white/5">
                    <div className="flex justify-between items-start">
                        <div className="flex items-center gap-3"><div className={`w-10 h-10 ${concept.color} bg-opacity-20 flex items-center justify-center rounded-lg border border-white/5`}><Disc size={20} className={concept.accent} /></div><div><p className="font-bold text-sm">Track {i}</p><p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest">Artist Name</p></div></div>
...
const StreamPreview = ({ concept }) => (
    <div className="flex flex-col h-full bg-[#050510] font-sans">
        ...
        <div className="flex-1 p-10 flex flex-col gap-10 overflow-y-auto">
            <div className={`h-48 rounded-[40px] ${concept.panel} p-10 flex flex-col justify-center relative overflow-hidden`}><div className="w-full h-10 bg-black/40 rounded-2xl relative overflow-hidden border border-white/5"><div className={`absolute top-1/2 left-0 w-full h-[2px] bg-white/5`}></div><div className={`absolute top-0 bottom-0 left-1/2 w-px bg-cyan-400 shadow-[0_0_15px_rgba(34,211,238,1)]`}></div></div></div>
