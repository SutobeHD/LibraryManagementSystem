...
const SettingsLibrary = ({ settings, setSettings }) => {
    ...
    const refreshWatcherStatus = useCallback(() => {
        api.get('/api/library/folder-watcher/status')
            .then((res) =>
                setWatcherStatus(res.data || { running: false, folders: [], pending_imports: 0 })
            )
            .catch(() => {});
    }, []);
    ...
    const addScanFolder = async () => {
        ...
        if (existing.includes(folder)) {
            toast.error('Folder already in list');
            return;
        }
        try {
    ...
    const removeScanFolder = async (path) => {
        try {
            ...
            const folders =
                res.data?.folders ?? (settings.scan_folders || []).filter((f) => f !== path);
            set('scan_folders', folders);
            refreshWatcherStatus();
        } catch {
            ...
            set(
                'scan_folders',
                (settings.scan_folders || []).filter((f) => f !== path)
            );
        }
    };

    const watchedSet = new Set(
        (watcherStatus.folders || []).filter((f) => f.alive).map((f) => f.path)
    );
        ...
        <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {['xml', 'live'].map((mode) => (
                        ...
                        >
                            ...
                            />
                            <span
                                className={`font-bold text-sm ${libStatus.mode === mode ? 'text-white' : 'text-ink-secondary'}`}
                            >
                                {mode === 'xml' ? 'XML Snapshot' : 'Live Database'}
                            </span>
                            <p className="text-[10px] text-ink-muted mt-1 text-center">
                                {mode === 'xml'
                                    ? 'Static Rekordbox export'
                                    : 'Direct access (master.db)'}
                            </p>
                        </button>
                    ))}
                </div>
            ...
            <Section title="Watched Folders" icon={FolderOpen}>
                <p className="text-xs text-ink-muted">
                    Folders monitored in the background. New audio files are auto-imported and
                    analysed as soon as they appear (or on next app start). An initial scan runs
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="px-3 py-2 bg-amber2/20 hover:bg-amber2/30 border border-amber2/30 rounded-xl text-amber2 transition-all"
                    >
                        <Plus size={16} />
                    </button>
                </div>
                    ...
                    <div className="space-y-2 mt-1">
                        {(settings.scan_folders || []).map((f) => {
                            ...
                            return (
                                <div
                                    key={f}
                                    className="flex items-center justify-between p-2.5 bg-mx-shell/60 rounded-xl border border-white/5"
                                >
                                    <div className="min-w-0 flex-1 flex items-center gap-2">
                                        ...
                                        />
                                        <span className="text-xs text-ink-primary font-mono truncate">
                                            {f}
                                        </span>
                                        <span
                                            className={`text-[10px] uppercase tracking-wide ${watching ? 'text-emerald-300' : 'text-ink-muted'}`}
                                        >
                                            {watching ? 'Live' : 'Idle'}
                                        </span>
                                    </div>
                                    <button
                                        onClick={() => removeScanFolder(f)}
                                        className="ml-2 flex-shrink-0 text-ink-muted hover:text-red-400 transition-colors"
                                    >
                                        <X size={14} />
                                    </button>
                                </div>
                ...
                {watcherStatus.pending_imports > 0 && (
                    <p className="text-[11px] text-amber2 mt-1">
                        {watcherStatus.pending_imports} file
                        {watcherStatus.pending_imports === 1 ? '' : 's'} queued for import…
                    </p>
                )}
            </Section>
