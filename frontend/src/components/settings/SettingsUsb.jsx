...
import { HardDrive, RefreshCw, Trash2, ChevronRight } from 'lucide-react';
...
const SettingsUsb = () => {
    ...
    const updateUsbProfile = useCallback(async (profile, patch) => {
        try {
            ...
            setUsbProfiles((prev) =>
                prev.map((p) => (p.device_id === profile.device_id ? updated : p))
            );
    ...
    return (
        <div className="space-y-4">
            <Section title="USB Export Profiles" icon={HardDrive}>
                ...
                {usbProfilesLoading ? (
                    <div className="text-tiny text-ink-muted py-4 text-center">
                        Loading profiles…
                    </div>
                ) : usbProfiles.length === 0 ? (
                    <div className="text-tiny text-ink-muted py-6 text-center">
                        No USB profiles yet. Plug in a USB stick and configure it from{' '}
                        <strong>USB Export</strong>.
                    </div>
                ) : (
                        ...
                        {usbProfiles.map((profile) => {
                            ...
                            return (
                                <div
                                    ...
                                    className="bg-mx-input rounded-mx-sm border border-line-subtle"
                                >
                                    {/* Row header */}
                                    <button
                                        onClick={() =>
                                            setEditingProfileId(isOpen ? null : profile.device_id)
                                        }
                                        ...
                                        <div className="flex items-center gap-3 min-w-0 flex-1">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="text-[12px] font-medium text-ink-primary truncate">
                                                    {profile.label ||
                                                        profile.drive ||
                                                        profile.device_id.slice(0, 12)}
                                                </div>
                                                <div className="text-[10px] text-ink-muted truncate">
                                                    {USB_TYPE_LABELS[profile.type] ||
                                                        profile.type ||
                                                        'Collection'}
                                        ...
                                        <div className="px-3 py-3 border-t border-line-subtle space-y-3">
                                            ...
                                            <Field label="Label">
                                                <input
                                                    ...
                                                    onBlur={(e) => {
                                                        if (
                                                            e.target.value !== (profile.label || '')
                                                        ) {
                                                            updateUsbProfile(profile, {
                                            ...
                                            <Field label="Type">
                                                <Select
                                                    ...
                                                    onChange={(v) =>
                                                        updateUsbProfile(profile, { type: v })
                                                    }
                                                    options={Object.entries(USB_TYPE_LABELS).map(
                                                        ([id, label]) => ({ id, label })
                                                    )}
                                                />
                                            </Field>
                                            ...
                                            <div className="pt-2 border-t border-line-subtle">
                                                <button
                                                    onClick={() =>
                                                        deleteUsbProfile(profile.device_id)
                                                    }
