        ...
        <div className="space-y-6">
            <Section title="DAW Keyboard Shortcuts" icon={Keyboard}>
                <p className="text-xs text-ink-muted">
                    Click any shortcut to capture a new key binding. Changes apply immediately in
                    the DAW editor after saving. Press{' '}
                    <kbd className="px-1.5 py-0.5 bg-mx-card rounded text-[10px] font-mono border border-white/10">
                        Esc
                    </kbd>{' '}
                    to cancel capture.
                </p>
                <div className="space-y-1.5">
                    {Object.entries(SHORTCUT_LABELS).map(([action, label]) => (
                        <div
                            key={action}
                            className="flex items-center justify-between p-2.5 rounded-xl hover:bg-mx-card/50 transition-colors"
                        >
                ...
                <button
                    onClick={() =>
                        setSettings((prev) => ({ ...prev, shortcuts: { ...SHORTCUT_DEFAULTS } }))
                    }
