        ...
        <div className="space-y-6">
            <Section title="Output Device" icon={Music}>
                <p className="text-xs text-ink-muted">
                    Select which audio output device the DAW playback engine uses. Takes effect the
                    next time a track is loaded.
                    {!window.__TAURI__ && (
                        <span className="block mt-1 text-amber-400">
                            ⚠ Device enumeration requires the desktop app (Tauri).
                        </span>
                    )}
                </p>
                <Field label="Audio output">
                    ...
                    >
                        {audioDevices.map((d) => (
                            <option key={d} value={d === 'System Default' ? '' : d}>
                                {d}
                            </option>
                        ))}
                    </select>
                ...
                {audioDevices.length === 1 && window.__TAURI__ && (
                    <p className="text-xs text-ink-muted italic">
                        Only the system default was found. Check that your audio drivers are
                        installed.
                    </p>
                )}
            </Section>
