        ...
        <div className="space-y-6">
            <Section title="Waveform Colors" icon={Palette}>
                <p className="text-xs text-ink-muted">
                    Custom colors for the 3-band waveform display (Low / Mid / High frequency
                    bands). Changes the palette across the DAW editor and overview.
                </p>
                <Field label="Color mode">
                    <select
                        value={settings.waveform_visual_mode || 'custom'}
                        onChange={(e) => set('waveform_visual_mode', e.target.value)}
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[
                        ...
                        {
                            key: 'waveform_color_high',
                            label: 'High (Air)',
                            defaultColor: '#3b82f6',
                        },
                    ].map(({ key, label, defaultColor }) => (
                        <div key={key} className="flex flex-col items-center gap-2">
                            <label className="text-xs text-ink-secondary font-bold uppercase">
                                {label}
                            </label>
                            <div className="relative">
                                <input
                                    ...
                                    style={{
                                        outline: `2px solid ${settings[key] || defaultColor}40`,
                                    }}
                                />
                            </div>
                            <span className="text-[10px] text-ink-muted font-mono">
                                {settings[key] || defaultColor}
                            </span>
                        </div>
                    ))}
                </div>
                <div className="flex items-center gap-3 h-6 rounded-lg overflow-hidden border border-white/10">
                    <div
                        ...
                        style={{ background: settings.waveform_color_low || '#ef4444' }}
                    />
                    <div
                        className="flex-1 h-full"
                        style={{ background: settings.waveform_color_mid || '#22c55e' }}
                    />
                    <div
                        className="flex-1 h-full"
                        style={{ background: settings.waveform_color_high || '#3b82f6' }}
                    />
                </div>
            </Section>
            ...
            <Section title="Language" icon={Globe}>
                <div className="grid grid-cols-2 gap-3">
                    {[
                        ...
                        >
                            ...
                            <span
                                className={`font-bold text-sm ${(settings.locale || 'de') === lang.id ? 'text-white' : 'text-ink-secondary'}`}
                            >
                                {lang.label}
                            </span>
                        </button>
                ...
                <p className="text-xs text-ink-muted italic">
                    Full i18n support is being rolled out progressively.
                </p>
            </Section>
        </div>
    );
