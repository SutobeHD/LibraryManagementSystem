...
const WaveformEditorInner = forwardRef(
    (
        {
            track,
            blobUrl = null,
        ...
        useEffect(() => {
            isPlayingExternalRef.current = isPlayingExternal;
        }, [isPlayingExternal]);
        ...
        const revokeAllBlobUrls = useCallback(() => {
            blobUrlsRef.current.forEach((u) => {
                try {
                    ...
                    log.debug('WaveformEditor revokeObjectURL failed', e);
                }
            });
            blobUrlsRef.current = [];
        }, []);
        ...
        useEffect(() => {
            (async () => {
                try {
                    ...
                    console.warn('Failed to load waveform settings', e);
                }
            })();
        }, []);
        ...
        const beats = useMemo(
            () => computeBeats(beatGrid, bpm, duration),
        ...
        if (loading && !fullTrack)
            return (
                <div className="flex h-full items-center justify-center text-ink-muted bg-black">
                    Loading...
                </div>
            );
        ...
        return (
            <div className="rb-edit-container relative">
                ...
                <WaveformBottomPanels
                    ...
                    loopOut={loopOut}
                    setLoopIn={setLoopIn}
                    setLoopOut={setLoopOut}
                    setIsLooping={setIsLooping}
                    handleSetLoopIn={interactions.handleSetLoopIn}
                    handleSetLoopOut={interactions.handleSetLoopOut}
                    selectedBeats={selectedBeats}
                    ...
                    handleCopy={interactions.handleCopy}
                    handlePaste={interactions.handlePaste}
                    handleInsert={interactions.handleInsert}
                    handleDelete={interactions.handleDelete}
                    handleClear={interactions.handleClear}
                    handleUndo={interactions.handleUndo}
                    handleRedo={interactions.handleRedo}
