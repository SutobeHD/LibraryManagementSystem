...
export async function loadAudio(filepath) {
    ...
    try {
        ...
        if (!response.ok && !useBackend) {
            console.warn(
                `[DawEngine] Asset URL fetch failed (${response.status}), retrying with backend...`
            );
        ...
        if (!response.ok) {
            console.error(
                `[DawEngine] Final fetch failed: ${response.status} ${response.statusText}`
            );
            throw new Error(`Failed to fetch audio: ${response.status} ${response.statusText}`);
        }

        log.debug(
            '[DawEngine] Fetch success, content-length:',
...
export async function renderTimeline(
    ...
    for (const region of sorted) {
        ...
        const sourceDur =
            region.sourceDuration || (region.sourceEnd || 0) - (region.sourceStart || 0);
