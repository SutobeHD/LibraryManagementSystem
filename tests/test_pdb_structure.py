...
def main():
    with tempfile.TemporaryDirectory(prefix="pdbtest_") as tmp:
        ...
        contents, artists, albums, keys, genres, labels, playlists, pl_entries = (
        ...
        print(
            f"  magic={magic} page_size={ps} num_tables={n_tab} next_unused={next_unused} u10={u10} seq={seq}"
        )
        ...
        for d in descriptors:
            ...
            while cur and cur not in visited:
                ...
                if cur == d["last"]:
                    ...
                    if next_page != d["empty"]:
                        print(
                            f"  Table type={d['type']}: last page {d['last']} next={next_page}, expected own empty {d['empty']}"
                        )
        ...
        for i, d in enumerate(descriptors):
            print(
                f"  [{i:2}] type={d['type']:2} empty={d['empty']:3} first={d['first']:3} last={d['last']:3}"
            )
...
def test_export_pdb_invariants():
    """pytest entry point — all export.pdb byte invariants must hold.

    Wraps the script `main()` (returns 0 on PASS) so CI's `pytest tests/`
    exercises the produced USB PDB, not just manual `python tests/...` runs.
    """
    assert main() == 0
