...
| `config/constants.js` | Frontend-wide tunables: `HEARTBEAT_INTERVAL_MS`, `LIBRARY_STATUS_INTERVAL_MS`, `RENDER_API_TIMEOUT_MS`, `BLOB_URL_REVOKE_DELAY_MS`, `TOAST_DURATION_LONG_MS`, `AUDIO_IMPORT_TIMEOUT_MS`, `IMPORT_TASK_POLL_INTERVAL_MS`, `ARTIST_MERGE_POLL_INTERVAL_MS`, `ARTIST_MERGE_MAX_POLL_FAILURES`, `ARTIST_LONG_OP_TIMEOUT_MS`, `ARTIST_CATALOGUE_TIMEOUT_MS`, `ARTIST_DOWNLOAD_POLL_INTERVAL_MS`, `ARTIST_DOWNLOAD_MAX_POLL_FAILURES`, `ARTIST_CATALOGUE_PAGE_SIZE`, `ARTIST_DISCOVER_TIMEOUT_MS`, `ARTIST_DISCOVER_LIMIT`, `ARTIST_SYNC_RUN_TIMEOUT_MS`, `ARTIST_SYNC_STATUS_POLL_MS`, `SC_REFRESH_TIMEOUT_MS` (headroom over the backend's 15 s SoundCloud call — explicitly not a guarantee, which is why the refresh classification keys on the `auth_expired` marker and a client abort reads as transient). |
...
| File | Purpose |
|------|---------|
| `api/api.js` | **Central Axios instance — always import from here, never use raw `fetch()`.** 10s timeout, session-token injection, 401 silent refresh with request queue (prevents parallel re-auth), 429 exponential backoff, automatic Tauri-context detection. `withCredentials: false` — Bearer-in-header is the only auth transport, there is no cookie-auth path. The 401 refresh verdict comes from `api/scRefreshClassification.js` and is tagged onto the rejection as `_refreshOutcome`, so the interceptor never re-classifies: only a genuinely expired SoundCloud login dispatches `sc:auth-expired`, a dead LMS session token and a transient network failure each take their own path. |
| `api/scRefreshClassification.js` | **What a failed `POST /api/soundcloud/refresh` meant** — pure predicate, split out of `api.js` so the verdict is testable without the axios instance (importing `api.js` runs `_bootstrap()` and touches `window`). `classifyRefreshError(err)` returns one of three exported constants: `SC_EXPIRED` (`sc-expired` — 401 whose body is `detail: 'auth_expired'`; SoundCloud rejected the stored refresh token, the keyring entry is already cleared ⇒ re-consent), `SESSION_DEAD` (`session-dead` — any other 401; `require_session` rejected our own SESSION_TOKEN after a sidecar restart, SoundCloud untouched ⇒ restart the app), `TRANSIENT` (`transient` — everything else incl. 503/429/network drop/client timeout/no response at all; the stored login is intact ⇒ retryable, and deliberately not counted against the refresh-loop guard). The route being `require_session`-gated is why the two 401 senders must be told apart — opening the OAuth window on a dead bearer would ask the user to fix the wrong thing. `api.js` tags the rejection with `_refreshOutcome` so the interceptor never re-classifies. |
| `api/scRefreshClassification.test.js` | `node --test frontend/src/api/scRefreshClassification.test.js` — 8 cases, no resolver needed (pure predicate, the import carries its extension). Covers the three verdicts and the load-bearing ones: only the `auth_expired` marker may open the OAuth window, a bare 401 reads as our own dead bearer, and anything without a 401 response stays transient. |
...
| `components/ArtistHubView.jsx` | **Artist Hub** — the Artists tab. Left pane: favourites (avatar, canonical name, local track count, SC-link chip, Auto/Review/Off segmented control, per-artist Update, un-favourite). Right pane: three-tab panel — "From your library" = Tier-1 backlog (owned artists by track count desc, favourites excluded, Add button); "All artists" = the complete library artist list from `GET /api/artists/browse`, server-side search + A–Z / most-tracks sort, per-row star that toggles the favourite optimistically (rolls back + toasts on failure) and refreshes the favourites pane and the backlog with it, `showing N of M` header + Load more paging (never silently truncated); "Discover" is **live** (`GET /api/artists/discover`): candidate rows carry the measured follower/track counts (an unmeasured count is dropped, never rendered as 0), which of your favourites pointed at them, an Add button that favourites them, and — above the list — the per-source note, so a failed / budget-capped / never-queried source can never read as "nothing found". **Update all** is now **Sync now**: it runs the same idle background pass on demand (`POST /api/artists/sync/run` with `force`), reports the run record it came back with, and renders a refusal ("No pass ran — a SoundCloud download is running") as a refusal. The Auto/Review/Off control no longer carries the "nothing acts on them yet" caveat: the scheduler honours it. Each favourite row shows when it was last checked (a never-synced artist shows nothing), and the panel prints the live idle sentence, the last run summary and — while any load path stays unobservable — a line saying "idle" is a reading, not a proof. The per-row **Update** is live for a linked artist: it forces a refresh of that artist's catalogue and toasts the counts it came back with; unlinked rows keep the button disabled with the reason. Clicking any row opens `artistHub/ArtistDetail` — the local `TrackTable` (library names resolved via `GET /api/artists`, alias variants fetched and de-duplicated) beside the SoundCloud missing-track split, with `ArtistDetailSummary` / `ArtistDetailActions` rendered into this view's own header. A **Duplicates · N** button in the header (and a per-row context-menu entry, seeded with that artist) opens `artistHub/MergeDialog`; `artistHub/ProjectionPanel` sits under the suggestion panel and owns the Rekordbox `Artists` folder. A merge, a revert or a favourite change invalidates the cached artist index and re-reads hub, browse, duplicate count and projection state. Routes to: `lib-artists`. | `GET /api/artists/hub`, `GET /api/artists/browse`, `POST /api/artists/favourites`, `DELETE /api/artists/favourites/{collection_id}`, `POST /api/artists/{collection_id}/sync-mode`, `GET /api/artists`, `GET /api/artist/{id}/tracks`, `GET /api/artists/merge/candidates` (badge), `GET /api/artists/{collection_id}/catalogue?refresh=true` (per-row Update) — `GET /api/artists/discover`, `GET /api/artists/sync/status` (polled while the tab is open), `POST /api/artists/sync/run` — merge + projection calls via `artistHub/artistHubApi.js`, catalogue + link + download via `artistHub/artistCatalogueApi.js`, discovery + sync via `artistHub/artistDiscoveryApi.js` |
...
| `components/artistHub/artistCatalogueApi.js` | **Transport for the SoundCloud catalogue + batch download routes.** `fetchCatalogue` (axios timeout disabled — a cold fetch pages through SoundCloud with spacing sleeps), `linkSoundCloudProfile` / `unlinkSoundCloudProfile`, `startMissingDownload` and `pollDownloadJob` (cadence + failure budget from `config/constants.js`; a 404 is terminal = the sidecar lost the job). `catalogueErrorMessage()` turns a failure into a sentence and translates the literal `auth_expired` 401 body into "the SoundCloud session expired" before it can reach a toast. The catalogue read is a **union on `status`**, not an error channel — callers must branch, never treat a typed state as an empty list. `pinTrackRole` posts a hand-picked role (`null` unpins) and `fetchTrackIdentities` reads the local artist→track table. API: `GET /api/artists/{id}/catalogue`, `POST\|DELETE /api/artists/{id}/link`, `POST /api/artists/{id}/tracks/{urn}/role`, `GET /api/artists/{id}/identities`, `POST /api/artists/{id}/download-missing`, `GET /api/artists/download/status`. |
| `components/artistHub/catalogueCopy.js` | **Pure copy + derivation for the artist detail** (no React, no network, unit-tested). `BUCKETS` is the render order of the six role buckets; `splitCatalogue` derives their rows, counts and missing counts — only an explicit `in_library:false` counts as missing (`null` = never diffed), and `queueable` mirrors the server's own `auto_queue_allowed` across every bucket so the button count equals what the backend will queue. `sourceStates` / `sourceStatusLine` / `allSourcesOk` / `bucketEmptyNote` carry the per-source honesty rule (successor to `repostsNote`, widened to uploads + search + reposts): an empty bucket may only say "nothing missing" when **every** source came back `ok`, otherwise the line names the sources that never ran. Also `roleLine` / `creditLine` (why a row is in its bucket, plus "pinned by you"), `ROLE_OPTIONS` for the pin menu, `searchNamesLine`, `LINK_MISSING_CHIP` / `LINK_MISSING_SENTENCE`, `downloadAllNote`, `stateSentence`, `fetchedLine`, `truncationNote`, `callBudgetLine`, `progressLine` and `downloadSummary` / `downloadTone` (downloaded · skipped · failed — a partial run never gets a success toast). |
| `components/artistHub/catalogueCopy.test.js` | `node --test frontend/src/components/artistHub/catalogueCopy.test.js` — 22 cases, incl. "a typed state derives empty buckets", "`in_library:null` is not missing", "an empty bucket may only claim nothing-missing when every source ran", "a missing source status defaults to not_queried, never ok" and "a run with failures is not a success". |
| `components/artistHub/useArtistCatalogue.js` | **Catalogue state for one selected artist.** Reads the catalogue when an artist is selected and `enabled` is true (the ToU guardrail: favourited/linked artists only, never speculatively), keeps typed states as successful answers instead of errors, sequence-stamps every response so a slow read for the previous artist cannot land in this one's panel, and owns `link` / `unlink` / `download` (job started + polled to the end, then the catalogue is re-read so the split reflects what landed) plus `pinRole`. `pinRole` moves the row between buckets optimistically through the exported pure `movePinnedRow` and restores the whole previous view if the server refuses, so the screen never shows a classification the sidecar did not store; clearing a pin is not predicted client-side (only the classifier knows where the row goes back to) and re-reads instead. Switching artist cancels the watch — nothing is reported about a run it stopped watching. |
| `components/artistHub/useArtistCatalogue.test.js` | `node --import ./frontend/src/components/artistHub/useArtistCatalogue.test.resolver.mjs --test frontend/src/components/artistHub/useArtistCatalogue.test.js` — 6 cases over one batch download's lifecycle, driven by a fake hooks runtime instead of React. The regression that shipped: `download()` guarded its `downloading`/`job` reset with `load`'s fetch counter, which every catalogue re-read bumps, so a finished run — or an Update pressed mid-run — left the panel disabled with the progress bar pinned and no result summary. Also covers "the panel stays busy until the closing re-read lands", "an earlier run finishing does not reset a later one" and "switching artist mid-run writes nothing into the new artist". Needs its own `--import` resolver, which must **not** be co-loaded with the dawReducer one — a second `node --test` command, never a shared run. Shims (never run directly): `useArtistCatalogue.test.resolver.mjs` registers `…resolver-impl.mjs`, which points `react` at the fake runtime and `artistCatalogueApi` at the stub and retries extensionless relative specifiers with `.js`; `…test.api-stub.mjs` stands in for `artistCatalogueApi` (the real one pulls in axios and `import.meta.env`, which raw Node cannot load) and records what the hook asked for; `…test.fake-react.mjs` supplies `renderHook` / `settle` over a minimal hooks runtime (`useState`/`useRef`/`useMemo`/`useCallback`/`useEffect`). |
| `components/artistHub/useArtistDetailActions.js` | **The detail view's click handlers**, shared by the header controls and the panel body: link (`promptModal`, never `prompt()`), unlink (`confirmModal`), Update (forced refresh, toasts the real counts or the typed state), Download all missing (`confirmModal` carrying `downloadAllNote`, which states that only `primary`/`remixer` rows at high/medium confidence are queued and that every review bucket has to be pulled row by row), per-track download and `handlePinRole` (toasts the new role, or the rollback). Every finished run is reported through `downloadSummary`/`downloadTone`. |
| `components/artistHub/ArtistDetail.jsx` | **Screen 2 of the mockup — owned vs missing.** Left panel: the shared `TrackTable` (passed in as children, `variant="embedded"`) with an alias chip. Right panel: "Missing on SoundCloud" tabbed by **role**, in the owner's order — Their tracks · Their remixes · Remixed by others · Featured on · Uncertain — review — each tab showing `missing/total` and its rule as a tooltip. Every row carries duration, post date, uploader, the parsed credit (`roleLine` + `creditLine`: why it landed here and at what confidence, "pinned by you" when the user overruled the classifier), a per-track Download and a **Role** pin menu that moves the row immediately and rolls back with a toast if the sidecar refuses. Under the tabs, the per-source status line ("Uploads ✓ · Search ✓ · Reposts not queried (budget)") plus which spellings were searched; an empty bucket renders `bucketEmptyNote`, so a bucket nobody queried never reads as "nothing missing". A preview-only track renders as "Not streamable" and is not downloadable. Below: the collapsed **Mixes & sets (N) — excluded** strip stating the 15-min/keyword rule with a per-row reason, expandable so a single one can still be pulled. A live progress bar during a run, then a result row (downloaded · skipped · failed + the first errors), a cache/fetched-at chip, the call-budget line, the truncation warning, and — for an unlinked artist — the amber "not linked — identified by name only" chip beside a prominent Link action. Exports `ArtistDetailSummary` + `ArtistDetailActions` for the hub's own header. Every no-list branch routes through one empty state, so a typed backend state can never render as an empty bucket. |
...
| `components/artistHub/artistDiscoveryApi.js` | **Transport for the discovery + background-sync routes.** `fetchDiscovery` (axios timeout disabled — one `/related` call per linked favourite with the client's own spacing), `fetchSyncStatus` (read-only, unauthenticated, safe to poll) and `runBackgroundSync({force})`. `isSyncAlreadyRunning` separates the 409 "a pass is in flight" from a real failure, and `discoveryErrorMessage` reuses the catalogue routes' 401/429 vocabulary. Contract note in the header: the discovery payload is never a bare list, and a `sync/run` answer is a **report**, not a success flag — a pass refused for load comes back 200 with `reason_stopped: "busy:<probe>"`. API: `GET /api/artists/discover`, `GET /api/artists/sync/status`, `POST /api/artists/sync/run`. |
| `components/artistHub/discoveryCopy.js` | **Pure copy builders for the Discover tab + the background-sync line** (no React, no network, unit-tested). `relatedNote` / `coOccurrenceNote` / `allSourcesAnswered` / `emptyNote` carry the honesty rule: an empty list may only read as "nobody found" when BOTH sources answered `ok`. `exclusionNote` refuses to claim "already yours" when `excluded_against` is empty; `candidateFacts` drops a `null` count instead of printing 0; `callBudgetNote` prints the per-run cap straight from the payload; `seedLine` names the favourites that pointed at a candidate. For sync: `busyReasonSentence` turns a probe reason (`sc_download:downloading`) into a sentence, `idleSentence` explains the wait, `runSummary` reports only counted numbers and renders a refusal as a refusal, `lastSyncedLabel` stays empty for a never-synced artist, and `SYNC_MODE_HINTS` states that Auto never downloads. `syncStateNote(last_error)` renders the two non-failure markers — `SYNC_PARTIAL_PREFIX` (`'partial: '`, the `#<n>` attempt count parsed off into the title) and `SYNC_CAPPED_PREFIX` (`'capped: '`) — as ' (partial)' / ' (capped)' in amber, and only a bare error as ' (failed)' in red; both prefixes must stay byte-identical to `app/artist_store/sync.py`'s `PARTIAL_PREFIX` / `CAPPED_PREFIX`. `STATE_SKIPPED_TRACK_CAP` + `SKIPPED_TRACK_CAP_NOTE` name the skip a fresh call budget cannot undo. |
| `components/artistHub/discoveryCopy.test.js` | `node --test frontend/src/components/artistHub/discoveryCopy.test.js` — 26 cases, incl. "a budget-cut marker reads as partial, not failed", "a not-queried source is never silence", "an empty list only reads as nobody-found when both sources answered", "already yours is not claimed when nothing was checked against", "an unmeasured count is dropped, not rendered as zero" and "a refused pass never reads as a sync". |
...
| `components/settings/SettingsNetwork.jsx` | HTTP proxy, SoundCloud sync target, **Artist Background Sync** (`artist_background_sync`, default off — one line stating it only runs while the app is open and idle, only over Auto/Review favourites, capped at 20 artists / 60 calls per pass, and never downloads), backend restart. Hidden "expert" toggle (5× click on muted dot reveals aggressive-download switch). |
