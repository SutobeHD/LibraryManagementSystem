...
|------|---------|--------------|
| `components/MetadataView.jsx` | Library browser — mode (`playlists`/`labels`/`albums`) driven by the `mode` prop from the Library workspace nav (no own title/tab bar). Playlists → `PlaylistBrowser`; labels/albums → item grid → `TrackTable`. `lib-artists` is **not** its route any more (see `ArtistHubView`); `main.jsx` holds the last non-artist mode so it doesn't reload behind the hub. Routes to: `lib-playlists\|labels\|albums`. | `GET /api/labels\|albums`, `GET /api/{type}/{id}/tracks`, `POST /api/metadata/merge` |
| `components/ArtistHubView.jsx` | **Artist Hub** — the Artists tab. Left pane: favourites (avatar, canonical name, local track count, SC-link chip, Auto/Review/Off segmented control, per-artist Update, un-favourite). Right pane: suggestion panel, tab "From your library" = Tier-1 backlog (owned artists by track count desc, favourites excluded, Add button); tab "Discover on SoundCloud" **renders disabled** — SoundCloud sync is a later milestone, and Update / Update all are disabled for the same reason with the reason stated in the panel, never stubbed. Clicking a favourite or backlog row drills into that artist's local tracks in the shared `TrackTable` (library names resolved via `GET /api/artists`, alias variants fetched and de-duplicated). Routes to: `lib-artists`. | `GET /api/artists/hub`, `POST /api/artists/favourites`, `DELETE /api/artists/favourites/{collection_id}`, `POST /api/artists/{collection_id}/sync-mode`, `GET /api/artists`, `GET /api/artist/{id}/tracks` |
