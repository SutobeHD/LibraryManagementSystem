...
| `components/artistHub/artistCatalogueApi.js` | **Transport for the SoundCloud catalogue + batch download routes.** `fetchCatalogue` (axios timeout disabled — a cold fetch pages through SoundCloud with spacing sleeps), `linkSoundCloudProfile` / `unlinkSoundCloudProfile`, `startMissingDownload` and `pollDownloadJob` (cadence + failure budget from `config/constants.js`; a 404 is terminal = the sidecar lost the job). `catalogueErrorMessage()` turns a failure into a sentence and translates the literal `auth_expired` 401 body into "the SoundCloud session expired" before it can reach a toast. The catalogue read is a **union on `status`**, not an error channel — callers must branch, never treat a typed state as an empty list. API: `GET /api/artists/{id}/catalogue`, `POST\|DELETE /api/artists/{id}/link`, `POST /api/artists/{id}/download-missing`, `GET /api/artists/download/status`. |
| `components/artistHub/catalogueCopy.js` | **Pure copy + derivation for the artist detail** (no React, no network, unit-tested). `splitCatalogue` derives the three buckets and the counts — only an explicit `in_library:false` counts as missing (`null` = never diffed), and `queueable` mirrors the server's own `auto_queue_allowed` so the button count equals what the backend will queue. Also `stateSentence` (typed backend states, backend `detail` first), `fetchedLine` (cache hit vs live fetch + when), `truncationNote`, `callBudgetLine`, `progressLine`, `downloadSummary` / `downloadTone` (downloaded · skipped · failed — a partial run never gets a success toast) and the mix/set + remix rule sentences. |
| `components/artistHub/catalogueCopy.test.js` | `node --test frontend/src/components/artistHub/catalogueCopy.test.js` — 13 cases, incl. "a typed state derives empty buckets", "`in_library:null` is not missing" and "a run with failures is not a success". |
| `components/artistHub/useArtistCatalogue.js` | **Catalogue state for one selected artist.** Reads the catalogue when an artist is selected and `enabled` is true (the ToU guardrail: favourited/linked artists only, never speculatively), keeps typed states as successful answers instead of errors, sequence-stamps every response so a slow read for the previous artist cannot land in this one's panel, and owns `link` / `unlink` / `download` (job started + polled to the end, then the catalogue is re-read so the split reflects what landed). Switching artist cancels the watch — nothing is reported about a run it stopped watching. |
| `components/artistHub/useArtistDetailActions.js` | **The detail view's click handlers**, shared by the header controls and the panel body: link (`promptModal`, never `prompt()`), unlink (`confirmModal`), Update (forced refresh, toasts the real counts or the typed state), Download all missing (`confirmModal` stating that only the artist's own uploads are queued) and per-track download. Every finished run is reported through `downloadSummary`/`downloadTone`. |
| `components/artistHub/ArtistDetail.jsx` | **Screen 2 of the mockup — owned vs missing.** Left panel: the shared `TrackTable` (passed in as children, `variant="embedded"`) with an alias chip. Right panel: "Missing on SoundCloud" with the owner's segmented split — **Definitely theirs** (uploader-URN match, the only bucket "Download all missing" queues) and **Remixes by others** (named in title/tags, uploaded by someone else, downloadable one by one) — paged by `ARTIST_CATALOGUE_PAGE_SIZE`, each row carrying duration, post date, uploader and a per-track Download; a preview-only track renders as "Preview only" and is not downloadable. Below: the collapsed **Mixes & sets (N) — excluded** strip stating the 15-min/keyword rule with a per-row reason, expandable so a single one can still be pulled. A live progress bar during a run, then a result row (downloaded · skipped · failed + the first errors), a cache/fetched-at chip, the call-budget line, and the truncation warning when the fetch was cut short. Exports `ArtistDetailSummary` + `ArtistDetailActions` for the hub's own header. Every no-list branch routes through one empty state, so a typed backend state can never render as an empty bucket. |
