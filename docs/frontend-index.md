...
| `components/UtilitiesView.jsx` | Tools + Library Health view — `mode` prop (`tools`/`health`) driven by the workspace nav (no own toggle). Tools lazy-loads `PhraseGeneratorView`, `DuplicateView`, `XmlCleanView`. Library Health surfaces low-quality / lost / no-artwork tracks (Low Quality/Lost/No Cover sub-filters). Routes to: `util-tools` / `util-health`. | `GET /api/insights/low_quality`, `GET /api/insights/no_artwork`, `GET /api/insights/lost` |
