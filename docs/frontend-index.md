...
| File | Purpose |
|------|---------|
| `main.jsx` | App root — single unified top bar (brand + workspace tabs + library status + Settings/Exit). Workspaces: Library · Audio Import · Ranking · Editor · Sync · Utilities. Per-workspace views render in an in-content `WorkspaceNav` strip (shown when a workspace has >1 view); the Library strip carries metadata modes (Playlists/Artists/Labels/Albums) as `lib-<mode>` tabs, the Utilities strip carries one flat tab per tool + per Library-Health filter (Phrase Cues / Duplicates / XML Cleaner / Converter / Low Quality / Lost / No Cover, as `util-<mode>`) plus Insights. Mode picker, error boundary, lazy-loaded tab router (Suspense), heartbeat + library-status polling, session-token injection. |
...
| `components/UtilitiesView.jsx` | **Router on `mode`** (from the workspace nav, no inner grid/toggle). Tools: lazy-loads `PhraseGeneratorView` / `DuplicateView` / `XmlCleanView` / `FormatConverterView`. Health: `low_quality`/`lost`/`no_artwork` → track list. Routes to: `util-<mode>` (phrase/duplicates/xml/converter/low_quality/lost/no_artwork). | `GET /api/insights/low_quality`, `GET /api/insights/no_artwork`, `GET /api/insights/lost` |
| `components/FormatConverterView.jsx` | **Library format converter** — scope picker (all_m4a / playlist id / folder path) + target (AIFF/FLAC/WAV/MP3), dry-run preview (counts + size + disk warn/abort), Convert (ConfirmModal-gated) with live progress polling, per-run Roll back (ConfirmModal-gated). `useToast` errors. Lazy-loaded inside `UtilitiesView` (`converter` tab). | `POST /api/library/format-swap`, `GET /api/library/format-swap/status/{task_id}`, `POST /api/library/format-swap/rollback` |
