...
| `audio/analysis.rs` | `compute_waveform()`, `estimate_bpm()`, `detect_key()`, `hz_to_bin()` | RustFFT-based waveform computation. 3-band frequency split with **Hz-based band boundaries** (20-250-4000-20000 Hz, sample-rate-correct via `hz_to_bin()` — previously hardcoded bin indices broke at non-44.1k sample rates). BPM tempo detection. **`detect_key()` is a stub** that returns the empty string — real key detection lives in `app/analysis_engine.py` (Essentia). See `TODO(rust-key-detect)` in the function doc-comment. |
