...
One row per test case. Layer ∈ py/js/integration/perf.

| ID | Layer | Test file | Case | Covers |
|---|---|---|---|---|
| T1 | py | `tests/test_format_converter.py::test_engine_ports_row_mutation` | engine mutate folder_path/file_name/file_type/file_size → update_content, no delete+readd | Step 1, Migration |
...
| T3 | py | `tests/test_format_converter.py::test_aiff_codec_args` | AIFF cmd → pcm_s16le/s24le, -ar locked source SR, no resample | Step 3 |
...
| T5 | py | `tests/test_format_converter.py::test_probe_bit_depth_sample_fmt` | ffprobe sample_fmt s16→16, s32/s24→24; bits_per_raw fallback | Step 4, OQ6 |
| T6 | py | `tests/test_format_converter.py::test_disk_preflight_abort_15x` | est=src×ratio; free<1.5×→400 abort, <1.2×→warn flag | Step 5, OQ4 |
...
| T16 | py | `tests/test_format_converter.py::test_ffmpeg_arg_list_no_shell` | crafted filename → arg-list shell=False, no injection | CI-1 |
| T17 | py | `tests/test_format_converter.py::test_huge_batch_disk_abort_before_write` | 5.5× expansion → 400 abort, zero files written | DOS-1 |
| T18 | py | `tests/test_format_converter.py::test_rekordbox_running_returns_409` | `_is_rekordbox_running` true → 409, lock not bypassed | TI-1 |
...
- [x] **T-1 — Shared request model.** `app/format_swap_models.py` — `FormatSwapReq`/`FormatSwapScope`/`FormatSwapOptions`/`FormatSwapRollbackReq` (exactly-one-scope `model_validator`, target+trigger enum `field_validator`, `mp3_quality` 0-9, `scope_dict()` helper). — covers Step 7/10, tests T8,T23. **DONE** (commit `c6fccc7`). ⚠ ruff-clean + AST-valid but NOT import-verified here (pydantic absent).
...
- [x] **T-3 — Engine probe + codec + disk preflight.** Pure logic in `app/format_swap_codec.py` (`build_ffmpeg_cmd`, `parse_bit_depth`, `estimate_target_bytes`, `disk_verdict`); `ffprobe` 30s + ffmpeg 600s wiring as module functions `_probe_sample_rate`/`_probe_bit_depth`/`_run_ffmpeg` in `app/format_converter.py`. — covers Steps 3/4/5/6, tests T3,T4,T5,T6,T7,T16. **DONE** (commit `1fd322d`). NOTE: fixed a real `disk_verdict` bug — abort 1.5x > warn 1.2x made the warn band unreachable; now monotonic (abort <1.2x, warn 1.2x–1.5x, ok ≥1.5x).
- [x] **T-4 — Engine swap + snapshot + manifest.** `FormatSwapEngine.run` — batch `db_lock()`, RB-relaunch watchdog every 50, atomic anchored manifest, per-track `update_content` row mutation (content_id preserved, no delete+readd), per-track DB-fail recovery, tracker updates. — covers Steps 1/2, tests T1,T2,T12,T20. **DONE** (commit `1fd322d`; 16 engine tests via fake rbox handle + monkeypatched ffmpeg, no rbox/FFmpeg needed).
- [x] **T-5 — Engine rollback.** `FormatSwapEngine.rollback(manifest_id)` — restore DB+WAL+SHM, rename `.backup-<ts>` audio back, delete new files; `manifest_id` basename-guarded (Threat FS-3). — covers Step 9, tests T19,T22. **DONE** (commit `1fd322d`).
- [x] **T-6 — POST /api/library/format-swap route.** `app/main.py`: `Depends(require_session)`, mode≠live 400, dir-scope resolve+`is_relative_to` sandbox (FS-1), `_is_rekordbox_running` 409, disk-abort 400; `dry_run=True` sync plan, else spawn daemon thread + `{task_id}`. — covers Step 7, tests T8,T9,T13,T14,T15,T17,T18,T21,T23. **DONE** (commit `c6fccc7`). ⚠ NOT runtime-verified (fastapi absent) — ruff-clean + AST-valid.
- [x] **T-7 — GET status + POST rollback routes.** `GET .../status/{task_id}` (tracker lookup, 404), `POST .../rollback` (`require_session`, RB-running 409, basename-guard via engine). — covers Steps 8/9, tests T10,T19,T22. **DONE** (commit `c6fccc7`). ⚠ NOT runtime-verified.
- [~] **T-8 — Sister bilateral wiring.** **Contract READY** (commit `c6fccc7`): `format_swap_models.FormatSwapReq` is the shared importable shape; route + engine accept `trigger="quality_verdict"` (pass-through, logged). REMAINING: the sister's `/api/quality/swap/request` `{track_id, candidate_id}` → engine call lands in the `library-quality-upgrade-finder` feature when it ships. — covers Step 10, test T11.
...
- [~] **T-10 — Phase-1b chaos drill.** **Engine-level DONE + verified**: `test_partial_abort_then_rollback_restores_only_converted` — mid-run abort ("Rekordbox is running") → partial manifest → `rollback()` restores exactly the converted track + the DB snapshot, leaving the aborted/unreached tracks untouched (`tests/test_format_converter.py`, runs without rbox/FFmpeg). REMAINING: the 10-track *real-audio* scale variant (FFmpeg + real master.db). — covers Step 12, test T25.
...
- [~] **T-12 — Perf tests.** `tests/test_format_converter_perf.py`: **status-lookup latency (<10ms p95) + dry-run-plan latency (5000 rows <2s) DONE + verified** (fake handle, no FFmpeg); throughput (≥10× realtime) + peak-RSS (≤150MB) are `skipif(no ffmpeg)` scaffolding for a full runner. — covers Perf rows, tests T27–T31.
...
### 2026-05-31 (later) — blocker RESOLVED; engine T-3/T-4/T-5 shipped (commit `1fd322d`)

- **Proof-script blocker cleared.** `scripts/dev/safe_format_swap.py` IS present (committed `d2631da` "add safe_format_swap.py dev utility (proof artefact)"; the doc's `fdb461c` SHA was wrong but the file exists). Engine ported from it faithfully.
- **`app/format_converter.py` `FormatSwapEngine` built (T-4 + T-5 + T-3 probe wiring).** Generalised m4a→AIFF to any source → AIFF/FLAC/WAV/MP3 via the codec module. rbox handle + master.db path + backup dir **injected**; process helpers (`_probe_sample_rate`/`_probe_bit_depth`/`_run_ffmpeg`/`_is_rekordbox_running`/`_kill_rekordbox_if_present`) module-level.
- **Genuinely tested here** (despite no rbox/FFmpeg): `tests/test_format_converter.py` — 16 tests drive the full swap + per-track DB-fail recovery + ffmpeg-fail safety + RB-running/disk aborts + rollback on a tmp filesystem with a `FakeMasterDb` + monkeypatched ffmpeg. Verified content_id is preserved (mutate-in-place, never delete+readd) and originals are always recoverable. 34 tests total green; ruff + mypy clean.
- **Bug found + fixed:** `disk_verdict` had `ABORT=1.5 > WARN=1.2` → the warn band was unreachable (abort always fired first). Reconciled to monotonic bands (abort <1.2×, warn 1.2×–1.5×, ok ≥1.5×). The research doc's loose "1.5× hard-abort" is now the 1.5× *comfort* line; **confirm this reconciliation at review** (the OQ4 wording is ambiguous).
- **Still deferred (need a runner with deps + your local test):** T-1 model (pydantic), T-6/T-7 routes (fastapi), T-8 sister wiring, T-9/T-10/T-11 empirical drills (FFmpeg + real AAC + RB export-XML), T-12 perf, T-13/T-14 frontend. The PROVISIONAL Rekordbox FileType codes still need byte-verification vs a real master.db before `implemented_`.

### 2026-05-31 (later still) — API layer T-1/T-6/T-7 + T-8 contract (commit `c6fccc7`)

- **T-1 `app/format_swap_models.py`** — Pydantic v2 `FormatSwapReq`/`Scope`/`Options`/`RollbackReq`. `model_validator` enforces exactly-one scope selector; `field_validator` uppercases+validates target, validates trigger; `mp3_quality` bounded 0-9; `scope_dict()` carries `trigger` for the engine log marker. Standalone (no app imports) so the sister doc imports the same shapes (Gap 5).
- **T-6/T-7 routes in `app/main.py`** — `POST /api/library/format-swap` (require_session; `db.mode!="live"`→400; user `path` scope resolved + `is_relative_to(ALLOWED_AUDIO_ROOTS)` → FS-1; `dry_run` synchronous plan; real run = RB-running 409 + disk-abort 400 + `format_swap_tracker.register` + daemon `threading.Thread` running `build_engine_from_live_db().run(...)`), `GET .../status/{task_id}` (tracker, 404), `POST .../rollback` (require_session, RB 409, FormatSwapError→400).
- **`build_engine_from_live_db()` factory** added to `format_converter.py` — resolves `app.database.db.active_db.db` (rbox handle) + `db.live_db_path`; **called on the worker thread** because rbox uses a per-thread connection (resolving on the request thread then using on the daemon thread would cross connections).
