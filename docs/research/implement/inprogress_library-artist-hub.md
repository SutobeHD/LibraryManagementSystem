...
- 2026-09-26 — `implement/inprogress_` — **owner refinement 2** (interactive): *"beim Downloaden einen Erkenner, ob es den Artist gibt und Song linked etc und Name korrekt ist."* Gap check: every download runs through `soundcloud_downloader`; tags = `publisher_metadata.artist` or uploader username + title as uploaded — no credit parse, no store/library lookup, so a label upload lands as `Boysnoize Records — Boys Noize - Starter` and a re-cased name becomes a new spelling; the catalogue diff never reads the registry's `sc_track_id → local_track_id`, so a track downloaded from "missing" can stay missing when titles differ. Tasks T-28..T-32 added (M5). Non-goals :98 + :102 amended (owner decision).
...
- **Download recognizer** (owner, 2026-09-26 (2)): between the downloader's metadata fetch and its tag write, decide who the track is by and what the file says. Credited artist: label metadata > title prefix (`X - Title`) > uploader. Known? — against the artist store (canonical, aliases, favourites, bound SC account) and the library's artists, `fold_key`-tolerant, ambiguous = unknown. Written to the tags only when HIGH: prefix repeats the credited artist, prefix names a known artist, a known artist's own spelling, uploader = the artist's bound account. Anything else = suggestion on the download, applied by the user. Each credit (primary / remixer / featured) reported as known + favourite + library count, role relative to that artist. Linked = correct tags (attribution + projection pick it up) + the catalogue counts a registry-linked download as owned. **Metric:** `Boys Noize - Starter` uploaded by `Boysnoize Records` with Boys Noize in the library → file tagged `Boys Noize — Starter`, page + playlist list it, catalogue shows it owned; same upload with an unknown prefix → tags untouched, one suggestion; `Overdrive - Extended Mix` never split.
...
- New download backend. This doc consumes the existing/planned downloader, it does not extend it. **Amended 2026-09-26 (2):** one hook in the existing downloader's tagging step (the recognizer). Still no new backend, no new source, save path unchanged.
...
- Rewriting the per-track `artist` **string** for reasons other than a merge — that is `metadata-name-fixer`'s job. (A merge itself **does** now rewrite tags — owner decision 2026-09-04.) **Amended 2026-09-26 (2):** the names a *new download* is born with are decided here (HIGH corrections only). Tracks already in the library stay the fixer's; a guess stays a suggestion the user applies — the fixer's never-auto-apply rule holds.
...
| T38 | js | `frontend/src/components/artistHub/linksCopy.test.js` (links + local copy) | service labels / source sentences / role groups never claim what the payload lacks | T-25, T-26 |
| T39 | py | `tests/test_artist_recognizer.py` | label upload naming a known artist → split, HIGH; unknown prefix → suggestion only, tags untouched; version phrase never split; prefix repeating the credited artist stripped; label metadata wins | T-28, Threat T18 |
| T40 | py | `tests/test_artist_recognizer.py` | known spelling / alias / bound uploader account → canonical; ambiguous fold → untouched; credits carry known / favourite / new + role relative to each artist | T-28, Goal |
...
- [x] **T-11:** absorb "By Artist" + one-way `metadata_mappings.json` import — Step 9, tests T12 — **DONE** shipped 2026-09-09 (`727abca`). `generate_smart_playlists` is labels-only; its artist branch grouped on the raw string and created unconditionally, so it made one folder per spelling and duplicated every playlist per re-run. An existing `By Artist` folder is reported (`legacy_by_artist`) and never deleted. **The mappings import turned out to be unnecessary**: `_normalize_artist_name` (`app/live_database.py:605`) consults `metadata_mappings.json` before `db.artists` is built, so the store already receives the mapped names — a second importer would only be able to drift from the first.
...
- [x] **T-11b:** USB relocation pass — a pure artist rename moves/merges folders on the stick instead of re-copying; case-only two-step rename; collision fallback — Step 5b, tests T25, T26, T27 — **DONE** shipped 2026-09-09 (`727abca`), `tests/test_usb_relocate.py`. Both branches gate on a content fingerprint (size + the whole file up to 128 KiB, above that both 64 KiB edges; `None` on `OSError` = unproven) — name + size alone also matches a re-exported file belonging to a track outside this sync, and the collision branch *deletes* from the stick. Fingerprint widened 2026-09-21: the first version hashed size + the head only for 64 KiB < size <= 128 KiB, so two different short recordings sharing an oversized ID3 cover-art frame matched (`test_a_shared_head_is_not_proof_for_a_small_file`). **Gap closed 2026-09-21:** `sync()`'s own Stage-1b call path is covered by `tests/test_usb_relocate.py::TestOneLibrarySyncStage1b` — pure rename copies zero bytes through `sync()`, plus `audio_copy=False`, playlist-filter, slot-count and `dest_resolver` bounds.
...
- [x] **T-19 (login):** `app/soundcloud_auth.py` — one atomic keyring blob {access, refresh, expires_at}; `get_access_token()` refreshes silently inside `min_ttl`, single-flight lock, `invalid_grant` → clear + `AuthExpiredError`, network error → keep old blob; Rust `login_to_soundcloud` returns the full triple instead of a bare string; every `KEYRING_SC_TOKEN` reader switches to the getter; `POST /api/soundcloud/refresh`; the 401 interceptor tries `/refresh` before `invoke('login_to_soundcloud')`. Implements `evaluated_soundcloud-persistent-login` Option A. Tests: `tests/test_soundcloud_auth.py`. — **DONE** shipped 2026-09-08 (`cb414fa`, `52771e9`). **The owner must log in once more** for a refresh token to exist; until then the store reports `refreshable: false, source: legacy`.
- [x] **T-20 (identity):** `search_tracks` / `search_tracks_many` on `GET /tracks?q=` (same paginator, same `CallBudget`, `access=playable`); `app/artist_store/identity.py` — `classify_roles` (primary / remixer / remixed_by_other / featured / uncertain × HIGH/MEDIUM/LOW, `fold_key` only), single pure `auto_queue_allowed`; sidecar migration adding `track_identity` (SC URN PK, **ISRC**, role, confidence, `user_override`); ISRC exact-match fast path in the owned/missing diff; catalogue fetch = uploads ∪ search(name+aliases) ∪ reposts under one budget with **per-source status**; role-pin route; UI buckets by role with credit-parse shown. Tests: `tests/test_artist_identity.py` + route tests. — **DONE** shipped 2026-09-08 (`cb414fa`, `52771e9`). `auto_queue_eligible` additionally refuses a match made **only** on the uploader name — a promo channel's name is not a credit.
...
- [x] **T-22:** social links backend — schema v3 `web_links` + `link_fetch`; `app/artist_store/links.py` (classify / precedence / hide); `app/musicbrainz_client.py` (shared, throttled); SC `get_user` + `get_user_web_profiles`; links routes — tests T33–T36 — **DONE** 2026-09-26 (`49eae3c`). Schema writers roll back on error since `189fc48` — a failed write used to leave the shared connection mid-transaction.
- [x] **T-23:** SC account suggestions — `search_users` + `GET …/soundcloud/candidates` (suggest only, linking stays a click) — tests T36 — **DONE** 2026-09-26 (`49eae3c`)
