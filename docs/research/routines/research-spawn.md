...
- Find it: `gh issue list --search "Idea Backlog in:title" --state open --json number,title`.
- Exists → overwrite its body with this run's proposals (`gh issue edit <num> --body ...`).
...
created `docs/research/research/idea_<slug>.md`. The routine never
creates `idea_*.md` files itself.
...
If at least one P0 / P1 proposal is **new** (not in the previous body), also post a short `gh issue comment` so the user gets a notification.
