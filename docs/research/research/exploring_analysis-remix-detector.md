...
- Classify each track with variant label from fixed enum `(original, extended, radio, club, dub, instrumental, acapella, vip, remix, bootleg, edit, mashup)` — **canonical order matches sister-doc `external-track-match-unified-module` Findings 2026-05-17 "canonical `VersionTag.label` enum" Literal block, "stem family before derivation family"** (order aligned for diff-grep; members identical). Target: ≥ 95 % precision on labelled fixture of 200 tracks drawn from real library; recall ≥ 80 % (untagged "Original Mix" assumed when no tail-parenthetical found).
