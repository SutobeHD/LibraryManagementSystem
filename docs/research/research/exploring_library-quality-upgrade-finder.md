...
- **Canonical `VersionTag.label` enum lives in `external_track_match`** — the 12-member `Literal["original","extended","radio","club","dub","instrumental","acapella","vip","remix","bootleg","edit","mashup"]` is owned by sister-doc `external-track-match-unified-module` (see its Findings 2026-05-17 "canonical `VersionTag.label` enum"). This doc is a **read-only consumer** of that enum — same-edit detection compares `VersionTag.label` between library track and candidate (refuse-by-default when labels differ); it never defines or extends the enum.
- **`Candidate` shape ack** — the frozen `Candidate(source: str, source_id: str, title: str, artist: str, duration_s: float | None, version_tag: VersionTag | None, url: str | None, raw: dict)` (sister-doc OQ10 RESOLVED-M1) is agreed + owned by `external_track_match`; this doc consumes it for replacement-candidate search and does not redefine it.
- **Rust-FP-via-IPC can substitute `fpcalc` for safety-rule 2** — per sister-doc `external-track-match-unified-module` Findings 2026-05-17 "Rust-FP-via-IPC" (final bullet): when `fpcalc` is missing AND both the library track and the replacement candidate are local files (the HQ-folder source scenario), safety-rule 2 (chromaprint-match required, threshold ≥ 0.95) may instead accept the in-house Rust fingerprint similarity (`hamming_similarity` ≥ 0.95) via the Tauri-direct-IPC path. This keeps the replace-flow usable when `fpcalc` is unavailable. Substitution applies only to local-vs-local pairs — external-URL candidates still require `fpcalc` (Rust FP is bit-incompatible with AcoustID). Does not relax Rule 2 for the non-local case.
