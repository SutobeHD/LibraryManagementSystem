...
- **Existing Rust fingerprint pipeline already in tree** — `src-tauri/src/audio/fingerprint.rs` ships a Chromaprint-style in-house fingerprint (32-band Mel × Goertzel, Hamming similarity, `fingerprint_track` / `fingerprint_batch` Tauri commands). Offline, no `fpcalc` shellout. NOT bit-compatible with AcoustID — cannot query AcoustID with Rust fingerprints. Two-tier fingerprint design needed (see Findings #3): Rust = local-cluster only; Python `fpcalc` = external lookup.
...
10. **Variant taxonomy enum vs free-tagged** — RESOLVED. Fixed `Enum` (12 values per Goals). Unknown variants fall through to `remix` (catch-all). Justification: sister-doc `external-track-match-unified-module` OQ1 calls for same — taxonomy is shared module's contract.
...
- **Deliverables:** Python wrapper around existing `fingerprint_batch` Tauri command; cluster job batches up to N=500 tracks per Tauri call; Hamming-similarity threshold calibration sweep on fixture (sweep 0.75–0.95 in 0.02 steps, pick precision-recall sweet spot); writes `track_variants` rows with `source='rust-fp-cluster'`, confidence 0.85 (above auto-group floor).
- **Covers:** untitled / mislabelled variant pairs Option A misses ("Track 04" + "Hidden Bonus" same recording).
- **Gates before M3:** (i) Hamming threshold validated on fixture, (ii) batch latency ≤ 1 s/track average, (iii) zero new dep entries in `requirements.txt` / `Cargo.toml`, (iv) cluster precision ≥ 95 % (false-positive cluster is worse than false-negative — merging two unrelated tracks into one cluster confuses UI).

### M3 — Opt-in Chromaprint + AcoustID + MusicBrainz (external)

- **Deliverables:** `pyacoustid==1.3.0` pinned in `requirements.txt`; `app/fingerprint_acoustid.py` with `shutil.which('fpcalc')` PATH-detect + import-time degrade; `app/musicbrainz_relations.py` querying `recording-recording` relations; background job batches via AcoustID `meta=recordings+releasegroups`; cache key `(fpcalc_fingerprint, duration_rounded)` in `variants.db`; `settings.json` `variant_detector.external_enabled` toggle (default `false`).
...
- Code (existing, M1 will consume): `app/soundcloud_api.py:566` (`_fuzzy_match_with_score`), `app/soundcloud_api.py:583` (threshold 0.65), `app/database.py:22` (`_db_write_lock`), `app/anlz_safe.py` (rbox quarantine — not used here, noted for completeness), `src-tauri/src/audio/fingerprint.rs` (M2 source, `fingerprint_track` + `fingerprint_batch` Tauri commands), `backend.spec` (verified no `fpcalc` reference)
