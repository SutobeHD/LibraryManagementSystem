...
- 2026-09-16 — `research/exploring_` — Idea-Verifier PASS (4 defects fixed in place: derived-goal label, static-playlist classification in OQ7, 2 citation corrections, download-folder migration note); promoted. Ready for `research-explore`
...
- **Removal policy (derived — not in the Original Idea; a mirror needs one rule or it silently diverges).** Unliked / removed-from-playlist → configurable (default: drop from the mirror playlist, keep file + library row); final shape = OQ5. **Metric:** 0 audio files deleted in any mode.
- **Genre + sub-genre routing.** SC `genre` + `tag_list` (+ `bpm` when present) → user's own vocabulary via an editable rule table → written to the fields the `Genres` smart lists key on (carrier = OQ7: Genre field / Comment chips / MyTag). Static playlists inside the `Genres` tree (screenshot: `Hardtechno`) are not reachable by field writes — smart-vs-static classification + what to do with static ones = OQ7. **Metric:** ≥ 90 % of a seeded 100-track corpus lands in the intended smart list with 0 manual tagging; 0 silently invented vocabulary (novel terms queue for review — D5 rule).
...
13. **Album-tag fallback.** Album = `publisher_metadata.album_title`, else the SC playlist title (`app/soundcloud_downloader.py:912`): keep / drop / move the fallback? A track in 3 playlists gets which? Owner.
...
- Agent 1 (empirical on a `master.db` copy + codebase): OQ7 + OQ16 — dump the `Genres` smart-list rules via `_parse_smart_rules`, map field / value → playlist; rbox MyTag write probe; RB re-evaluation; PDB genre / comment carry-through. Owner-approved copy, `scripts/dev/` probe like `rbox_artist_merge_probe.py`, shipped with a no-data fallback (synthetic smart-list XML) so explore never blocks on user data; OQ17 likewise owner-run + optional.
...
- **Intent fidelity:** every ask in `## Original Idea` maps to a Goal — Likes + playlists mirror → Goals 1 + 3; "only the new track" → Goal 2 (metric: exactly 1 download); SC genre taken along at download → auto Genre + Sub-genre → Goal 5 (+ Goal 6 seed); "extension of the Artist-Sync" → Prior Art parent + Goal 7. No dropped intent. Defect fixed: "Removal policy" was never asked — now labelled derived, final shape parked in OQ5. Defect fixed: static playlists inside `Genres` (`Hardtechno`) are unreachable by field writes — smart-vs-static classification added to Goal 5 + OQ7.
- **Prior-art handling:** parent / sibling / prereq boundaries confirmed against artist-hub (Non-goal quote, Steps 3 + 6, T-1 / T-3 / T-7 / T-12 / T-17) and downloader-unified (Q5-b, D5, `subgenres` column, D6). Defect fixed: downloader-unified Q6-b / D7 moves the download folder to `MUSIC_DIR/<artist>/` — noted in Constraints; the ledger never keys on a path.
- **Citations:** 21 spot-checked OK. Two corrected in place: `app/live_database.py:1342` is `add_track_to_playlist` — `remove_track_from_playlist` is `:1348-1357` (two-arg call `:1353`; drift inherited from artist-hub Step 1); `app/soundcloud_downloader.py:912` sets Album = `publisher_metadata.album_title` **or** the playlist title — wording softened everywhere.
- **Research Plan tractability:** 17 / 17 OQs covered, no orphans (A1 {7, 16}, A2 {1, 6, 14, 15}, A3 {2–5}, A4 {8, 9, 10, 13, 17}, A5 {11, 12}); each names its resolution form. Caveat carried into the plan: OQ7 / OQ16 / OQ17 need the owner's machine — probes ship with a no-data fallback, never block.
