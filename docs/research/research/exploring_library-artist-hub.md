...
- **SC API — the needed endpoints are NOT implemented.** Present: `/me` `app/soundcloud_api.py:279-292`, `get_user_profile` `:295-321`, `/resolve` `:359-402`, `/users/{id}/playlists` `:404-475`, `/users/{id}/favorites` (max 500) `:477-540`. **Missing: `/users/{id}/tracks`, followings, and any search** — search exists only in Rust (`src-tauri/src/soundcloud_client.rs:241`) and serves export, not discovery. Shared pager/retry to reuse: `_sc_get` `app/soundcloud_api.py:181`.
...
1. **RB folder idempotency + visibility.** Folder/playlist creation works (`app/database.py:992-996,1046-1060`). Open: does a re-sync delete-and-recreate or diff in place, does Rekordbox need `USN`/`rb_local_usn` bumping to survive a restart, does it show without a rescan? Resolvable: yes/no + working re-sync call.
2. **Does a per-track artist rewrite actually merge the entity?** No `DjmdArtist` writer exists; only `update_content_artist` per track (`app/live_database.py:1057-1061`). Open: after rewriting all N tracks, does the orphan `DjmdArtist` row vanish from RB artist browse or linger? If it lingers, which write removes it safely? Resolvable: empirical on a `master.db` copy.
3. **Merge blast radius.** Is `master.db` enough for RB display **and** USB export, or must ID3/AIFF tags (`app/audio_tags.py`), ANLZ, and the PDB artist rows (`PdbBuilder.add_artist`, `docs/MAP_L2.md:951`) change too? Resolvable: yes/no per surface.
4. **Undo-log reuse.** Does `app/metadata_fixer/schema.py` (`runs` + `mutations`, pre-image JSON, SHA-1, reverse replay) take artist-merge mutations as-is, or need an `entity_kind` column? A merge is N row-writes under one `run_id` — does revert ordering hold at N = 5000? Resolvable: schema diff + large-N revert test.
5. **SC artist catalogue.** `/users/{id}/tracks` is not implemented (only playlists + favorites exist). Open: page size, rate limit, does it include reposts/sets, how to exclude DJ-sets/podcasts from "missing". Resolvable: endpoint + limits + filter rule.
6. **SC related-artists.** Does the public SC API expose related/similar users at all? If not, derive Tier 2 from reachable data (followings of favourites, co-occurrence in playlists we already fetch, label mates). Resolvable: endpoint yes/no + named fallback algorithm.
...
- **SC client cannot fetch an artist's catalogue.** Implemented: `/me`, profile, `/resolve`, `/users/{id}/playlists`, `/users/{id}/favorites`. Missing: `/users/{id}/tracks`, followings, search (search exists only in Rust for export). OQ5/OQ6 are genuinely open and gate Goals 2 + 7.
