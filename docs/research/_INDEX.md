...
### inprogress
- [inprogress_library-format-converter.md](implement/inprogress_library-format-converter.md) — Library-wide format converter (Option A). **T-1..T-8 + T-13/T-14 SHIPPED** (commits `bfbe75d`/`1fd322d`/`c6fccc7`/`210f130`): engine (`format_converter.py`, 34 rbox-free tests) + Pydantic model + 3 `/api/library/format-swap` routes + React `FormatConverterView` wired into the Utilities `converter` tab. Fixed a `disk_verdict` unreachable-warn-band bug. **Remaining: T-9/T-10/T-11 empirical beatgrid drills (FFmpeg + real AAC + RB export-XML) + T-12 perf.** Routes/model/UI ruff/prettier-clean but NOT runtime-verified here (no pydantic/fastapi/node_modules) → owner local test + RB FileType byte-verify before `implemented_` (2026-05-31)
...
- [inprogress_db-write-lock-retrofit.md](implement/inprogress_db-write-lock-retrofit.md) — Close `_db_write_lock` coverage gaps. **ALL 5 TASKS SHIPPED** (commit `4eecafb` on `claude/research-continuation-U6R4p`): `app/_db_lock.py` `@serialise_mutators` prefix decorator on both DB classes (setattr name-list deleted), `AnalysisDBWriter` callsite `db_lock()`, `tests/test_concurrency.py` prefix-independent AST drift guard + serialisation harness (9 tests), FILE_MAP/MAP/CHANGELOG synced. Pending user sign-off + real-rbox harness run before `implemented_` (2026-05-31)
