...
- [evaluated_soundcloud-persistent-login.md](research/evaluated_soundcloud-persistent-login.md) — Persistent SoundCloud login; Recommendation = Option A (backend-owned silent refresh). **Shipped as artist-hub T-19 on `feat/artist-hub`** (`cb414fa`, `52771e9`, `app/soundcloud_auth.py`) — archive as `implemented_` once that branch merges (2026-09-09)
...
- [inprogress_library-artist-hub.md](implement/inprogress_library-artist-hub.md) — Artist Hub (approved 2026-09-04), built on `feat/artist-hub`. **Task queue CLOSED 2026-09-09** — M1 (`1cea04a` store/registry/hub, `0111e4c` All-artists tab, `61ab0d0` merge to master.db+tags + Rekordbox `Artists` folder), M2 (`5599b39` SC catalogue/split/batch download), M2b (`cb414fa` + `52771e9` T-19 persistent login, T-20 name-driven remix-aware identity), M3 (`727abca` T-11 By-Artist retirement, T-11b USB relocation, T-16 discovery, T-17 idle signal + background sync), T-18 doc sync (`ac6533e` + FILE_MAP/CHANGELOG/architecture). Each run adversarially verified by execution; four verify findings fixed pre-commit, two of them able to destroy audio on the stick. **Still `inprogress_` — promotion to `implemented_` needs owner sign-off.** Open gaps: the background-sync call budget is per pass, not per day (re-verified 2026-09-21 — fresh `CallBudget` per pass, no daily cap); the owner must log in to SoundCloud once for a refresh token to exist. **M4 re-opened 2026-09-26** (owner refinement: local attribution incl. remix credits + manual assign/exclude, social links from SC profile + MusicBrainz, links open in the desktop app) — T-21..T-26 done, T-27 index docs pending. **M5 added 2026-09-26** (owner refinement 2: download recognizer — known artist, linked song, correct names at download) — T-28..T-32 in flight. Branch `claude/loving-goodall-90uf7w`, PR #58 (2026-09-26)
