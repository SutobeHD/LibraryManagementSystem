...
- [approvalgate_library-artist-hub.md](implement/approvalgate_library-artist-hub.md) — ⛔ **AWAITING /approve** — Artist Hub: favourites list + per-artist owned/missing view, Rekordbox folder `Artists` (1 playlist per artist, idempotent), artist merge writing through to `master.db` (journalled + revertable), 2-tier suggestions, per-artist Update + idle background sync. Option A (sidecar store + projection engine); 18 tasks / 3 milestones, **M1 usable without SoundCloud**; 4 commit-blockers (`update_content` re-read in lock, playlist-removal arity fix, `_file_sha1` skip, USB cost in the merge dialog). Two owner decisions at the gate: USB re-copy after merge (OQ16), SC enumeration boundary (OQ15) (2026-09-04)
