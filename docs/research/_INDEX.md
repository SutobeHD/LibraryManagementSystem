...
### exploring
- [exploring_library-artist-hub.md](research/exploring_library-artist-hub.md) — Artist Hub: artist entity + favourites, RB folder `Artists` + 1 playlist/artist, merge writes through to `master.db` (journalled), 2-tier suggestions (local backlog → SC discovery), per-artist Update button + idle background sync. Stage 1 complete + V-PASS; wave-1 codebase scan banked. **3 half-built pieces found** (`generate_smart_playlists` By-Artist on raw strings, alias-only `/api/metadata/merge`, stubbed `/api/artist/soundcloud`) → consolidate, don't duplicate. Wave 2 closed OQ5+OQ6: `/users/{urn}/tracks` **and** `/users/{urn}/related` both exist in the public API (client just never called them); `_sc_get` refactor is task 1; new OQ15 = SC-ToU aggregation, owner decision at the gate. Wave 3 closed OQ3+OQ4 (master.db alone covers RB+USB; undo-log reusable with 3 deltas), OQ1+OQ2 PARTIAL — 7 empirical checks, 5 need no user data. **Live bug found:** `remove_track_from_playlist` calls rbox with wrong arity, always fails silently. New OQ16 = USB re-copy after merge. **Waves 4+5 closed OQ1+OQ2 empirically** (owner-approved run against a copy): rbox bumps USN itself on playlist creates and updates `masterPlaylists6.xml`; no playlist uniqueness at any level → own id-map; `update_content_artist` leaves `content.rb_local_usn` stale but `update_content(item)` bumps it → **merge writer decided**; `search_str` unpopulated → risk closed; live-bug arity reproduced + fix proven (2026-09-04)
