...
### inprogress
- [inprogress_library-format-converter.md](implement/inprogress_library-format-converter.md) — Library-wide format converter (Option A). **Foundation shipped** (T-2 tracker + T-3 pure codec/disk/probe logic, `app/format_swap_{tracker,codec}.py`, 19 tests green). Engine/routes/UI (T-1,T-4..T-14) deferred — env lacks rbox/FFmpeg/fastapi/node. **BLOCKER: proof script `scripts/dev/safe_format_swap.py` absent from checkout** (plan Step-1 premise). RB `FileType` codes provisional, need real-DB verify (2026-05-31)Artist/title normaliser; **T1 detector + T4 schema + T5 apply/revert engine SHIPPED** (`app/metadata_fixer/{detector,schema,applier}.py`, 25 tests green); T2/T3/T6–T10 `[ ]` for `research-implement` routine (2026-05-29)
