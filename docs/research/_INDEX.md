...
- [evaluated_soundcloud-persistent-login.md](research/evaluated_soundcloud-persistent-login.md) — Persistent SoundCloud login; Recommendation = Option A (backend-owned silent refresh). **Being implemented as artist-hub T-19 on `feat/artist-hub` (2026-09-08)** — archive as `implemented_` once that branch merges (2026-09-08)
...
### inprogress
- [inprogress_library-artist-hub.md](implement/inprogress_library-artist-hub.md) — Artist Hub (approved 2026-09-04), building on `feat/artist-hub`. **M1 + M2 SHIPPED** (`1cea04a` store/registry/hub, `0111e4c` All-artists tab, `61ab0d0` merge→master.db+tags + Rekordbox `Artists` folder, `5599b39` SC catalogue/split/batch download) — each run adversarially verified by execution. **2026-09-08 owner refinements in flight:** T-19 persistent login (silent refresh — root cause: refresh token parsed by Rust and dropped, never refreshed) + T-20 name-based identity with remix roles, search by name+aliases, `track_identity` table (ISRC/URN). Open: T-11 absorb By-Artist, T-11b USB folder merge, T-16/T-17 discovery + idle sync, T-18 doc sync (2026-09-08)
