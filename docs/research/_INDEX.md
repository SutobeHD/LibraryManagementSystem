...
### inprogress
- [inprogress_library-format-converter.md](implement/inprogress_library-format-converter.md) — Library-wide format converter (Option A). **T-2..T-5 SHIPPED** (commits `bfbe75d`+`1fd322d`): `app/format_swap_{tracker,codec}.py` + `app/format_converter.py` engine (scope/dry-run/swap/snapshot/manifest/rollback, content_id-preserving in-place row mutation), 34 tests green (engine driven via fake rbox handle + monkeypatched ffmpeg). Fixed a `disk_verdict` unreachable-warn-band bug. Remaining: T-1 model + T-6/T-7 routes (fastapi) + T-8 sister + T-9..T-11 empirical drills (FFmpeg/rbox) + T-12 perf + T-13/T-14 frontend. RB `FileType` codes provisional → real-DB verify before `implemented_` (2026-05-31)
