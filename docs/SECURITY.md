...
- **External links are untrusted until classified.** Artist profile links come from SoundCloud profiles, MusicBrainz, free bio text and pastes. `app/artist_store/links.py:classify_url` is the only way into `web_links`: http(s) only, no userinfo, no IP-literal or single-label host, default ports only, length-capped, control characters refused; the service is decided by an exact host-suffix table, so a lookalike host can at best be a generic website, and generic websites are accepted only from artist-curated or manual sources. The frontend re-checks with `frontend/src/utils/openExternal.js:safeExternalUrl` before asking the OS to open anything, and renders links as buttons, never as HTML. `tests/test_artist_links.py`, `frontend/src/utils/openExternal.test.js`.
- **`shell:allow-open` stays at the plugin's default scope.** `src-tauri/capabilities/main.json` grants the shell plugin's `open` command (added 2026-09-26 — until then every external link in the desktop app was silently refused). tauri-plugin-shell 2.3.5 validates the target against `^((mailto:\w+)|(tel:\w+)|(https?://\w+)).+` when `plugins.shell.open` is unset; do not widen it with a custom regex or a `with` program.
- **MusicBrainz is an outbound host (`musicbrainz.org`).** Calls go through `app/musicbrainz_client.py` only: one request per second per process (their policy is per IP — a second client would double the rate), a `User-Agent` naming the app and the repository — never the user —, one 503 retry, and MBIDs validated before they enter a request path. Lookups are user-initiated (Find links, confirming a match); nothing pre-fetches.
