...
| `frontend/src/components/UtilitiesView.jsx` | Tools + Library Health (`mode` prop from workspace nav). Tools grid: Phrase Cues, Duplicate Finder, XML Cleaner, Mass Format Converter (placeholder); sub-views open inline with back button. Library Health: Low Quality / Lost / No Cover track lists |
