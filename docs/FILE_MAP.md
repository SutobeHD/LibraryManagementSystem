...
| `app/usb_one_library.py` | `OneLibraryUsbWriter` — writes `PIONEER/rekordbox/exportLibrary.db` via rbox.OneLibrary so Rekordbox 6/7 auto-detects the stick. Optional `dest_resolver` constructor arg lets callers (`_sync_library_one`) reuse the legacy destination tree (`PIONEER/Contents/<Artist>/<Album>/file`) — without it tracks would land at `<USB>/Contents/...` and duplicate everything when both library types sync. Copies ANLZ sidecars to USBANLZ buckets, builds artist/album/genre/key/label caches, materialises playlist tree. |
