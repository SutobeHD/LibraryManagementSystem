...
| `app/soundcloud_api.py` | `SoundCloudPlaylistAPI` — SC unofficial v2 API with dynamic client_id scraping, exponential backoff, pagination. Added: `resolve_track_from_url()`, `download_url` in normalized track. `SoundCloudSyncEngine` — fuzzy title/artist matching. `AuthExpiredError`, `RateLimitError`. **Credential scrubbing is load-bearing:** `_scrub_secrets` / `_log_params` strip `client_id` / `oauth_token` / `access_token` / `secret_token` out of every logged URL, params dict and exception message, and `raise_for_status()` is replaced by a hand-built `HTTPError` (keeping `response=resp`) because `requests` renders the prepared URL into its own message. Pinned by `tests/test_soundcloud_log_redaction.py`. |
...
| `app/phrase_db_writer.py` | **Spike — nothing in the app calls it.** Writes phrase markers as Rekordbox **MEMORY cues** (`Kind=0`) into `master.db`'s `djmdCue` via pyrekordbox. `POST /api/phrase/commit` and the phrase batch worker still go through `phrase_generator.commit_phrase_cues`, which patches the ANLZ files only. Exists because ANLZ-only cues do **not** appear in Rekordbox's library view — Rekordbox reads cues from `djmdCue`, and the two drift (a track can carry a `djmdCue` memory cue while its ANLZ memory list is empty). Wiring it = call `write_phrase_memory_cues()` from `commit_phrase_cues` with ONE snapshot + ONE DB handle per batch; on the pinned surface each call otherwise costs a `SELECT ID` over every cue in the library (`_new_cue_ids`) — a batch caller fetches that set once and hands it down as `used=`. **Cannot open the live DB today:** `pyrekordbox==0.1.7` refuses Rekordbox >= 6.6.5 and needs a SQLCipher DBAPI plus the db key for the encrypted `master.db`; neither is installed (adding either is a dependency/secret decision), so `db_file=` must point at a decrypted copy. Safety contract: backs up the file it is about to mutate first (timestamped `.phrasebak-*` of that file + its `-wal`; `-shm` deliberately not copied), snapshot and write share ONE `db_lock()` hold, re-run REPLACES this track's own `P<n>` cues instead of duplicating (manual memory cues + hot cues never touched), `restore_master_db` rolls back a whole one-timestamp set and unlinks `-wal`/`-shm` first (a foreign WAL would replay the undone writes), Rekordbox open ⇒ `RekordboxLockedError` with no partial write. `InFrame = int(InMsec * 0.15)` (150 fps, verified vs real `djmdCue` rows); memory cues are point cues (`OutMsec=-1`, `OutFrame=0`). `_new_cue_ids` mints random IDs like Rekordbox, not `max()+1`; 0.1.7 has neither `db.add()` nor `generate_unused_id()`, so both are `getattr`-probed. Tests: `tests/test_phrase_db_writer.py` |
...
| `app/artist_store/sync.py` | The idle signal + the background catalogue refresh. **The idle signal** did not exist anywhere, so it is *composed* from the trackers that do: phrase batch, artist jobs, duplicate scan, SoundCloud downloader, local-import tracker, and the single-flight locks guarding USB sync / merge / projection / batch download. `is_idle()` returns `(bool, reason)` so the log and the UI can say **why** a pass did not run. Every probe **fails closed** — an exception, an unknown job status or an unreadable tracker means *busy*. One load path is genuinely unobservable and is named rather than hidden: `POST /api/library/analyze-batch` streams NDJSON and keeps no job record, so `idle_report()` lists it under `unobservable` and `register_probe()` is the hook to close that gap. **The background pass** refreshes every favourite whose mode is `auto` or `review` through the *caller's* refresher — the same route body the manual Update button uses, never a second fetcher — budget-cut refreshes first, then oldest-last-synced, under **one** shared `CallBudget`, re-checking idle between artists; `off` is skipped without a call. It **never downloads and never queues**: `downloads_queued` is in the run record and is always `0` so the invariant is observable, not merely asserted. Off by default (`artist_background_sync`). No HTTP, no credentials, no `master.db` here — the refresher owns all three. A refresh the shared budget cut short is **not** counted as synced: `stop_reason == "budget"` (or an already-exhausted budget) writes a `partial: #<n> <measured cause>` marker into `sync_state.last_error`, bumps the run record's `artists_partial` and sorts that artist first on the next pass — but only while its consecutive-cut count is below `MAX_CONSECUTIVE_PARTIALS` (2): cut short twice in a row and it keeps the marker yet rejoins the normal TTL order, so an artist the budget can never finish cannot own the head of the queue (and a 15-minute refetch loop) forever. A foreground refresh that left a complete catalogue clears the marker's effect — `_pending` believes the cached payload's `truncated` flag over a marker it has outlived. A truncation no retry can lift (the 2000-track ceiling, the fetcher's page/item caps) gets a `capped: <cause>` marker instead and **counts as a finished sync** — refetching it would spend quota for the same result. Both markers ride in `last_error` because it is the only per-artist status column the schema has; the Hub renders them "(partial)" / "(capped)", never "(failed)" (`artistHub/discoveryCopy.js:syncStateNote`). **Known limit:** the call budget is per pass, not per day — every pass builds a fresh `CallBudget(limit=60)` and the scheduler wakes every `ARTIST_SYNC_POLL_INTERVAL_S` (15 min); the only brake on repeat work is the per-artist 6 h TTL, which skips a fresh artist at zero calls. |
...
| `scripts/dev/scan_audio_quality.py` | ffprobe sweep over an audio root → JSON: one row per file (codec, stream/format bitrate, sample rate, channels, duration) + aggregate club-readiness buckets (`lossless` / `mp3_320` / `aac_256plus` / the lower MP3+AAC tiers / `unreadable`). Multiprocessing pool, default workers `max(2, min(8, cpu_count))` — ffprobe is seek-bound, more threads thrash an external USB/HDD. Needs ffprobe on PATH. Read-only: never touches the library DB or the files. `python scripts/dev/scan_audio_quality.py <root-dir> [--out report.json] [--workers N]` |
| `scripts/dev/rescan_unreadable.py` | Re-probes only the rows that carry an `error` in a `scan_audio_quality.py` report, merges the results back and rewrites the report in place (`--out` writes elsewhere). Use after unlocking files Rekordbox held open or fixing a path. Imports `probe`/`classify`/`default_workers` from its sibling, so the two cannot classify differently. Needs ffprobe on PATH. `python scripts/dev/rescan_unreadable.py [report.json] [--out rescanned.json] [--workers N]` |
