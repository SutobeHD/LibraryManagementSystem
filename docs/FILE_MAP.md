...
| `app/usb_pdb.py` | Header-only `export.pdb` + `exportExt.pdb` writer (legacy CDJ-2000nxs2 DeviceLibrary). Produces a structurally valid 8 KiB file (header + 20 empty table descriptors + sentinel index page). Row encoders are NOT implemented — older CDJ firmware will see the stick as Pioneer-formatted but with an empty library. Opt-in via `legacy_pdb_stub` setting (default off). Reference: https://djl-analysis.deepsymmetry.org/djl-analysis/pdb.html |
