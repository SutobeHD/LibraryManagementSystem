...
| `app/backup_engine.py` | Git-like incremental backup: compressed JSON changesets, HEAD tracking, commit timeline, restore. SQL identifiers are gated by `_TABLE_ALLOWLIST` (frozenset built from `TRACKED_TABLES`) + `_IDENT_RE` for column names — `_check_table_name()` raises `ValueError` on anything outside the static allowlist. Any new f-string SQL must call the validators before interpolation. |
