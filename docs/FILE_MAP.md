...
| `app/database.py` | `RekordboxXMLDB` — parses Rekordbox XML into an in-memory cache; `RekordboxDB` — **explicit facade** over `RekordboxXMLDB` / `LiveRekordboxDB`, selected by `mode`. **It has no `__getattr__`: a method missing from the facade is an `AttributeError` at the call site, not a passthrough.** Eight were missing while being called from routes until 2026-08-04 (`load_xml`, `save_xml`, `get_track_cues`, `get_track_beatgrid`, `save_track_cues`, `save_track_beatgrid`, `get_analysis_writer`, `get_unanalyzed_track_ids`) — see `docs/QUALITY_GATES.md`. Add new backing methods to the facade too; `mypy app/` is the guard. Cue/beatgrid **writes are XML-mode only** (live mode returns False — needs ANLZ sidecar rewrites). `get_all_labels`/`get_all_albums` memoise per instance via `_invalidate_group_caches()` — deliberately not `@lru_cache`, which keyed on `self` and pinned every library the sidecar ever loaded. There is no `get_tracks()`; it is `get_all_tracks()`. |
...
| `app/import_tracker.py` | Thread-safe per-file import-progress singleton (Queued → Analyzing → Importing → ANLZ → Completed/Skipped/Failed) backing `GET /api/import/tasks`. Also owns the worker-thread task binding: `bind_current_task()` / `unbind_current_task()` / `current_task()` over a module-level `threading.local()`. `services.write_companion_anlz` has no task_id in scope and reads it from there to report the ANLZ stage. This replaced an attribute monkey-patched onto `threading.current_thread()` (`._lms_import_tid`), which no type checker could see and which mutated a stdlib object shared process-wide. |
